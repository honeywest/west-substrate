...
mod tests {
    ...
    use node_runtime::{
        ...
        GenesisConfig, GrandpaConfig, Header, Log, Runtime, SessionConfig, StakingConfig, System,
        SystemConfig, UncheckedExtrinsic,
    };
    use primitives::{
        ed25519::{Pair, Public},
        twox_128, Blake2Hasher, ChangesTrieConfiguration,
    };
    use runtime_io;
    use runtime_primitives::traits::{Digest as DigestT, Header as HeaderT};
    use runtime_primitives::{
        generic, generic::Era, ApplyError, ApplyOutcome, ApplyResult, Perbill,
    };
    ...
    use wabt;
    use {balances, consensus, contract, session, staking, system, timestamp, treasury};
    ...
    fn sign(xt: CheckedExtrinsic) -> UncheckedExtrinsic {
        match xt.signed {
            Some((signed, index)) => {
                ...
                let pair = Pair::from(
                    Keyring::from_public(Public::from_raw(signed.clone().into())).unwrap(),
                );
                let signature = pair.sign(&payload.encode()).into();
                UncheckedExtrinsic {
                    signature: Some((
                        balances::address::Address::Id(signed),
                        signature,
                        payload.0,
                        era,
                    )),
        ...
        TestExternalities::new(
            GenesisConfig {
                ...
                balances: Some(BalancesConfig {
                    balances: vec![(alice(), 111), (charlie(), 100_000_000)],
                ...
                session: Some(SessionConfig {
                    session_length: 2,
                    validators: vec![
                        One.to_raw_public().into(),
                        Two.to_raw_public().into(),
                        three,
                    ],
                }),
                ...
                grandpa: Some(GrandpaConfig {
                    authorities: vec![
                        // set these so no GRANDPA events fire when session changes
    ...
    fn block2() -> (Vec<u8>, Hash) {
        construct_block(
            ...
            vec![
                // session changes here, so we add a grandpa change signal log.
                Log::from(::grandpa::RawLog::AuthoritiesChangeSignal(
                    0,
                    vec![
...
"#;
    ...
    fn deploying_wasm_contract_should_work() {
        ...
        let b = construct_block(
            ...
            vec![
                ...
                CheckedExtrinsic {
                    ...
                    function: Call::Contract(contract::Call::create::<Runtime>(
                        10.into(),
                        10_000.into(),
                        code_ctor_transfer,
                        Vec::new(),
                ...
                CheckedExtrinsic {
                    ...
                    function: Call::Contract(contract::Call::call::<Runtime>(
                        addr,
                        10.into(),
                        10_000.into(),
        ...
        WasmExecutor::new()
            .call(&mut t, 8, COMPACT_CODE, "Core_execute_block", &b.0)
            .unwrap();
    ...
    fn panic_execution_gives_error() {
        ...
        let foreign_code = include_bytes!(
            "../../runtime/wasm/target/wasm32-unknown-unknown/release/node_runtime.wasm"
        );
