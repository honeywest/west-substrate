...
use primitives::traits::{As, Hash, Zero};
use rstd::borrow::Borrow;
use rstd::prelude::*;
use runtime_io::print;
use srml_support::dispatch::Result;
use srml_support::{IsSubType, StorageMap, StorageValue};
use system::ensure_signed;
use {democracy, system};
...
impl<T: Trait> Module<T> {
    pub fn is_vetoed<B: Borrow<T::Hash>>(proposal: B) -> bool {
        Self::veto_of(proposal.borrow())
            .map(|(expiry, _): (T::BlockNumber, Vec<T::AccountId>)| {
    ...
    pub fn tally(proposal_hash: &T::Hash) -> (u32, u32, u32) {
        Self::generic_tally(proposal_hash, |w: &T::AccountId, p: &T::Hash| {
    ...
    fn generic_tally<F: Fn(&T::AccountId, &T::Hash) -> Option<bool>>(
        proposal_hash: &T::Hash,
        vote_of: F,
    ) -> (u32, u32, u32) {
        let c = <Council<T>>::active_council();
        let (approve, reject) = c
            .iter()
    ...
    fn end_block(now: T::BlockNumber) -> Result {
        while let Some((proposal, proposal_hash)) = Self::take_proposal_if_expiring_at(now) {
            ...
            {
                Self::deposit_event(RawEvent::TallyCancelation(
                    proposal_hash,
                    tally.0,
                    tally.1,
                    tally.2,
                ...
                Self::deposit_event(RawEvent::TallyReferendum(
                    proposal_hash.clone(),
                    tally.0,
                    tally.1,
                    tally.2,
                ...
                if tally.0 > tally.1 + tally.2 {
                    ...
                    <democracy::Module<T>>::internal_start_referendum(proposal, threshold, period)
                        .map(|_| ())?;
                }
            }
        }
...
mod tests {
    ...
    fn basic_environment_works() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_eq!(
                CouncilVoting::proposal_voters(H256::default()),
                Vec::<u64>::new()
            );
    ...
    fn set_balance_proposal(value: u64) -> Call {
        Call::Balances(balances::Call::set_balance(
            balances::address::Address::Id(42),
            value.into(),
            0.into(),
        ))
    }
    ...
    fn referendum_cancellation_should_work_when_unanimous() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_ok!(
                Democracy::internal_start_referendum(
                    proposal.clone(),
            ...
            assert_eq!(
                Democracy::active_referendums(),
                vec![(
                    0,
                    ReferendumInfo::new(4, proposal, VoteThreshold::SuperMajorityApprove, 0)
                )]
            );

            let cancellation = cancel_referendum_proposal(0);
            let hash = cancellation.blake2_256().into();
            assert_ok!(CouncilVoting::propose(
                Origin::signed(1),
    ...
    fn double_veto_should_not_work() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_noop!(
                CouncilVoting::veto(Origin::signed(2), hash),
    ...
    fn retry_in_cooloff_should_not_work() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            System::set_block_number(2);
            assert_noop!(
                CouncilVoting::propose(Origin::signed(1), Box::new(proposal.clone())),
                "proposal is vetoed"
            );
        });
    }
    ...
    fn unvoted_proposal_should_expire_without_action() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_eq!(
                CouncilVoting::tally(&proposal.blake2_256().into()),
                (1, 0, 2)
            );
            assert_ok!(CouncilVoting::end_block(System::block_number()));
    ...
    fn unanimous_proposal_should_expire_with_biased_referendum() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_ok!(CouncilVoting::vote(
                Origin::signed(2),
                proposal.blake2_256().into(),
                true
            ));
            assert_ok!(CouncilVoting::vote(
                Origin::signed(3),
            ...
            assert_eq!(
                ...
                vec![(
                    0,
                    ReferendumInfo::new(5, proposal, VoteThreshold::SuperMajorityAgainst, 0)
                )]
            );
        });
    }
    ...
    fn majority_proposal_should_expire_with_unbiased_referendum() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_eq!(
                ...
                vec![(
                    0,
                    ReferendumInfo::new(5, proposal, VoteThreshold::SimpleMajority, 0)
                )]
            );
        });
    }
