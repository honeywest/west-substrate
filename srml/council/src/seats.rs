...
impl<T: Trait> Module<T> {
    ...
    fn finalise_tally() -> Result {
        ...
        let incoming: Vec<T::AccountId> = leaderboard
            ...
            .cloned()
            .inspect(|a| {
                <balances::Module<T>>::unreserve(a, candidacy_bond);
            })
            .collect();
        let active_council = Self::active_council();
...
mod tests {
    ...
    fn candidate_submission_not_using_free_slot_should_not_work() {
        with_externalities(&mut new_test_ext_with_candidate_holes(), || {
            System::set_block_number(1);
            assert_noop!(
                Council::submit_candidacy(Origin::signed(4), 3.into()),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn bad_candidate_slot_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), Vec::<u64>::new());
            assert_noop!(
                Council::submit_candidacy(Origin::signed(1), 1.into()),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn non_free_candidate_slot_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), vec![1]);
            assert_noop!(
                Council::submit_candidacy(Origin::signed(2), 0.into()),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn poor_candidate_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), Vec::<u64>::new());
            assert_noop!(
                Council::submit_candidacy(Origin::signed(7), 0.into()),
    ...
    fn invalid_retraction_index_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_noop!(
                Council::retract_voter(Origin::signed(1), 1.into()),
    ...
    fn non_voter_retraction_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_noop!(
                Council::retract_voter(Origin::signed(2), 0.into()),
    ...
    fn simple_tally_should_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert!(Council::presentation_active());
            assert_eq!(
                Council::present_winner(Origin::signed(4), 2.into(), 20.into(), 0.into()),
                Ok(())
            );
            assert_eq!(
                Council::present_winner(Origin::signed(4), 5.into(), 50.into(), 0.into()),
                Ok(())
            );
            assert_eq!(
                Council::leaderboard(),
    ...
    fn double_presentations_should_be_punished() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            System::set_block_number(6);
            assert_ok!(Council::present_winner(
                Origin::signed(4),
                2.into(),
                20.into(),
                0.into()
    ...
    fn presenting_for_double_election_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            System::set_block_number(10);
            assert_noop!(
                Council::present_winner(Origin::signed(4), 2.into(), 20.into(), 1.into()),
    ...
    fn present_outside_of_presentation_period_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert!(!Council::presentation_active());
            assert_noop!(
                Council::present_winner(Origin::signed(5), 5.into(), 1.into(), 0.into()),
    ...
    fn present_when_presenter_is_poor_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_noop!(
                Council::present_winner(Origin::signed(1), 1.into(), 20.into(), 0.into()),
    ...
    fn invalid_present_tally_should_slash() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            System::set_block_number(6);
            assert_err!(
                Council::present_winner(Origin::signed(4), 2.into(), 80.into(), 0.into()),
                "incorrect total"
            );
