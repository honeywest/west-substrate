...
use codec::{Codec, Decode, Encode, HasCompact, Input, Output};
use primitives::traits::{
    As, BlockNumberToHash, CheckedAdd, CheckedSub, CurrentHeight, Lookup, MakePayment, Member, One,
...
impl<AccountId, X: EnsureAccountLiquid<AccountId>, Y: EnsureAccountLiquid<AccountId>>
...
impl<AccountId> EnsureAccountLiquid<AccountId> for () {
    fn ensure_account_liquid(_who: &AccountId) -> Result {
        Ok(())
    }
}
...
impl<T: Trait> Module<T> {
    ...
    pub fn set_free_balance_creating(
        who: &T::AccountId,
        balance: T::Balance,
    ...
    pub fn increase_free_balance_creating(
        who: &T::AccountId,
        value: T::Balance,
    ) -> UpdateBalanceOutcome {
    ...
    fn new_account(who: &T::AccountId, balance: T::Balance) -> NewAccountOutcome {
        ...
        let ret = {
            ...
            // period of time.
            let quantized_account_count: T::AccountIndex =
                (next_set_index * enum_set_size / quantization + One::one()) * quantization;
            // then modify the starting balance to be modulo this to allow it to potentially
            // identify an account index for reuse.
            let maybe_try_index = balance
                % <T::Balance as As<T::AccountIndex>>::sa(
                    quantized_account_count * reclaim_index_modulus,
                );
            ...
            if maybe_try_index % reclaim_index_modulus == reclaim_index_magic {
                ...
                if item_index < try_set.len() {
                    if Self::total_balance(&try_set[item_index]).is_zero() {
                        ...
                        Self::deposit_event(RawEvent::NewAccount(
                            who.clone(),
                            try_index,
    ...
    pub fn lookup(
        ...
        match a {
            ...
            address::Address::Index(i) => {
                <Module<T>>::lookup_index(i).ok_or("invalid account index")
            }
        }
    }
}
...
impl<T: Trait> MakePayment<T::AccountId> for Module<T> {
    fn make_payment(transactor: &T::AccountId, encoded_len: usize) -> Result {
        ...
        let transaction_fee = Self::transaction_base_fee()
            + Self::transaction_byte_fee() * <T::Balance as As<u64>>::sa(encoded_len as u64);
