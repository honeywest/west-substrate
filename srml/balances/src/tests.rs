    ...
    with_externalities(
        ...
        || {
            ...
            assert_ok!(Balances::transfer(
                ...
                5.into(),
                (256 * 20).into()
            )); // account 2 becomes zombie freeing index 1 for reclaim)
            assert_eq!(Balances::total_balance(&2), 0);
    ...
    with_externalities(
        ...
        || {
            ...
            assert_ok!(Balances::transfer(
                ...
                5.into(),
                (256 * 20 - 50).into()
            )); // account 2 becomes zombie freeing index 1 for reclaim) 50 is creation fee
            assert_eq!(Balances::total_balance(&2), 0);
...
fn balance_reduction_works() {
    with_externalities(&mut ExtBuilder::default().build(), || {
        ...
        assert_eq!(Balances::total_balance(&1), 42);
        assert_noop!(
            Balances::decrease_free_balance(&1, 69).map(|_| ()),
...
fn balance_transfer_when_reserved_should_not_work() {
    with_externalities(&mut ExtBuilder::default().build(), || {
        ...
        assert_ok!(Balances::reserve(&1, 69));
        assert_noop!(
            Balances::transfer(Some(1).into(), 2.into(), 69.into()),
...
fn transfer_overflow_isnt_exploitable() {
    with_externalities(&mut ExtBuilder::default().creation_fee(50).build(), || {
