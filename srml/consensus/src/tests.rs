...
use mock::{new_test_ext, Consensus, System};
use primitives::{
    generic, testing,
    traits::{OnFinalise, ProvideInherent},
};
...
fn authorities_change_logged() {
    with_externalities(&mut new_test_ext(vec![1, 2, 3]), || {
        ...
        assert_eq!(
            header.digest,
            testing::Digest {
                logs: vec![generic::DigestItem::AuthoritiesChange::<H256, u64>(vec![
                    4, 5, 6
                ]),],
            }
        );
...
fn authorities_change_is_not_logged_when_not_changed() {
    with_externalities(&mut new_test_ext(vec![1, 2, 3]), || {
        ...
        Consensus::on_finalise(1);
        let header = System::finalise();
        assert_eq!(header.digest, testing::Digest { logs: vec![] });
    });
}
