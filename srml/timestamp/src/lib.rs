...
use runtime_primitives::traits::{
    As, Block as BlockT, Extrinsic, ProvideInherent, SimpleArithmetic, Zero,
};
...
where
    A: OnTimestampSet<Moment>,
    B: OnTimestampSet<Moment>,
{
    fn on_timestamp_set(moment: Moment) {
...
impl<T: Trait> ProvideInherent for Module<T> {
    ...
    fn check_inherent<Block: BlockT, F: Fn(&Block::Extrinsic) -> Option<&Self::Call>>(
        ...
        let xt = block
            .extrinsics()
            .get(T::TIMESTAMP_SET_POSITION as usize)
            .ok_or_else(|| {
                CheckInherentError::Other("No valid timestamp inherent in block".into())
            })?;
        ...
        let t = match (xt.is_signed(), extract_function(&xt)) {
            ...
            _ => {
                return Err(CheckInherentError::Other(
                    "No valid timestamp inherent in block".into(),
                ))
            }
        }
        .into()
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Timestamp must increment by at least <BlockPeriod> between sequential blocks"
    )]
