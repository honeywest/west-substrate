            ...
            v.retain(|&index| {
                ...
                if let Some(p) = Self::proposals(index) {
                    if p.value <= budget_remaining {
                        ...
                        <balances::Module<T>>::increase_free_balance_creating(
                            &p.beneficiary,
                            p.value,
                        );
...
mod tests {
    ...
    fn spend_proposal_fails_when_proposer_poor() {
        with_externalities(&mut new_test_ext(), || {
            assert_noop!(
                Treasury::propose_spend(Origin::signed(2), 100.into(), Address::Id(3)),
    ...
    fn reject_already_rejected_spend_proposal_fails() {
        with_externalities(&mut new_test_ext(), || {
            ...
            assert_ok!(Treasury::reject_proposal(Origin::ROOT, 0.into()));
            assert_noop!(
                Treasury::reject_proposal(Origin::ROOT, 0.into()),
    ...
    fn accept_non_existant_spend_proposal_fails() {
        with_externalities(&mut new_test_ext(), || {
            assert_noop!(
                Treasury::approve_proposal(Origin::ROOT, 0.into()),
                "No proposal at that index"
            );
        });
    }
