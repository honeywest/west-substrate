...
fn staking_should_work() {
    with_externalities(&mut new_test_ext(0, 1, 2, 0, true, 0), || {
        ...
        assert_ok!(Staking::unstake(
            Origin::signed(4),
            (Staking::intentions().iter().position(|&x| x == 4).unwrap() as u32).into()
        ));
        assert_eq!(Staking::current_era(), 1);
        Session::check_rotate_session(System::block_number());
        ...
        assert_ok!(Staking::unstake(
            Origin::signed(3),
            (Staking::intentions().iter().position(|&x| x == 3).unwrap() as u32).into()
        ));
        Session::check_rotate_session(System::block_number());
        assert_eq!(Session::validators(), vec![1, 3]);
...
fn double_staking_should_fail() {
    with_externalities(&mut new_test_ext(0, 1, 2, 0, true, 0), || {
        ...
        assert_ok!(Staking::stake(Origin::signed(1)));
        assert_noop!(
            Staking::stake(Origin::signed(1)),
            "Cannot stake if already staked."
        );
        assert_noop!(
            Staking::nominate(Origin::signed(1), 1.into()),
            "Cannot nominate if already staked."
        );
        assert_ok!(Staking::nominate(Origin::signed(2), 1.into()));
        assert_noop!(
            Staking::stake(Origin::signed(2)),
            "Cannot stake if already nominating."
        );
        assert_noop!(
            Staking::nominate(Origin::signed(2), 1.into()),
...
fn staking_balance_transfer_when_bonded_should_not_work() {
    with_externalities(&mut new_test_ext(0, 1, 3, 1, false, 0), || {
        ...
        assert_ok!(Staking::stake(Origin::signed(1)));
        assert_noop!(
            Balances::transfer(Origin::signed(1), 2.into(), 69.into()),
            "cannot transfer illiquid funds"
        );
    });
}
...
fn slash_value_calculation_does_not_overflow() {
    with_externalities(&mut new_test_ext(0, 3, 3, 0, true, 10), || {
        ...
        // slashing.
        <ValidatorPreferences<Test>>::insert(
            10,
            ValidatorPrefs {
