...
pub struct ValidatorPrefs<Balance: HasCompact + Copy> {
    // TODO: @bkchr shouldn't need this Copy but derive(Encode) breaks otherwise
...
impl<T: Trait> Module<T> {
    ...
    fn slash_validator(v: &T::AccountId, slash: T::Balance) {
        ...
        if let Some(rem) = <balances::Module<T>>::slash(v, slash) {
            ...
            let total = noms
                .iter()
                .map(<balances::Module<T>>::total_balance)
                .fold(T::Balance::zero(), |acc, x| acc + x);
            if !total.is_zero() {
                let safe_mul_rational = |b| b * rem / total; // TODO: avoid overflow
                for n in noms.iter() {
                    let _ = <balances::Module<T>>::slash(
                        n,
                        safe_mul_rational(<balances::Module<T>>::total_balance(n)),
    ...
    fn reward_validator(who: &T::AccountId, reward: T::Balance) {
        ...
        let validator_cut = if reward.is_zero() {
            ...
            for n in noms.iter() {
                let _ = <balances::Module<T>>::reward(
                    n,
                    safe_mul_rational(<balances::Module<T>>::total_balance(n)),
                );
            }
    ...
    fn new_session(actual_elapsed: T::Moment, should_reward: bool) {
        if should_reward {
            ...
            let total_rewarded_stake =
                Self::stake_range().1 * <T::Balance as As<usize>>::sa(validators.len());
            ...
            let event = if slash_count >= grace {
                ...
                let slash = match base_slash.checked_shl(instances) {
                    ...
                    None => {
                        ...
                        base_slash.checked_shl(instances - 1).expect(
                            "slash count no longer incremented after overflow; \
                    ...
                    || Self::slashable_balance(&v) < next_slash
                {
                    if let Some(pos) = Self::intentions().into_iter().position(|x| &x == &v) {
                        Self::apply_unstake(&v, pos).expect(
                            "pos derived correctly from Self::intentions(); \
