...
use srml_support::{Dispatchable, IsSubType, Parameter, StorageMap, StorageValue};
    ...
    pub fn tally(ref_index: ReferendumIndex) -> (T::Balance, T::Balance, T::Balance) {
        Self::voters_for(ref_index)
            ...
            .map(|(bal, vote)| {
                if vote.is_aye() {
                    (
                        bal * T::Balance::sa(vote.multiplier() as u64),
                        Zero::zero(),
                        bal,
                    )
                } else {
...
mod tests {
    ...
    fn invalid_vote_strength_should_not_work() {
        with_externalities(&mut new_test_ext(), || {
            ...
            .unwrap();
            assert_noop!(
                Democracy::vote(Origin::signed(1), r.into(), Vote::new(true, 7)),
                "vote has too great a strength"
            );
            assert_noop!(
                Democracy::vote(Origin::signed(1), r.into(), Vote::new(false, 7)),
                "vote has too great a strength"
            );
        });
    }
    ...
    fn poor_seconder_should_not_work() {
        with_externalities(&mut new_test_ext(), || {
            ...
            assert_noop!(
                Democracy::second(Origin::signed(1), 0.into()),
