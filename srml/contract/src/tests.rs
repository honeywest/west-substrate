...
use wabt;
use {
    balances, runtime_io, system, CodeOf, ContractAddressFor, GenesisConfig, Module, RawEvent,
    StorageOf, Trait,
};
        ...
        assert_eq!(
            ...
            vec![
                EventRecord {
                    ...
                    event: MetaEvent::balances(balances::RawEvent::NewAccount(
                ...
                EventRecord {
                    phase: Phase::ApplyExtrinsic(0),
                    event: MetaEvent::contract(RawEvent::Transfer(
                        1,
                        CONTRACT_SHOULD_TRANSFER_TO,
                        6
                    )),
...
fn contract_transfer_to_death() {
    ...
    with_externalities(
        &mut ExtBuilder::default().existential_deposit(5).build(),
        || {
            <CodeOf<Test>>::insert(1, code_transfer.to_vec());
