...
pub use srml_metadata::{
    DecodeDifferent, StorageFunctionMetadata, StorageFunctionModifier, StorageFunctionType,
    StorageMetadata,
};
...
pub trait Storage {
    ...
    /// it's not there.
    fn require<T: codec::Codec>(&self, key: &[u8]) -> T {
    ...
    fn get_or_default<T: codec::Codec + Default>(&self, key: &[u8]) -> T {
    ...
    /// Take a value from storage, deleting it after reading.
    fn take_or_panic<T: codec::Codec>(&self, key: &[u8]) -> T {
        self.take(key).expect("Required values must be in storage")
    }

    /// Take a value from storage, deleting it after reading.
    fn take_or_default<T: codec::Codec + Default>(&self, key: &[u8]) -> T {
