...
pub fn decl_storage_impl(input: TokenStream) -> TokenStream {
    ...
    let hidden_crate_name = hidden_crate
        .map(|rc| rc.ident.content)
        .map(|i| i.to_string())
        .unwrap_or_else(|| "decl_storage".to_string());
    let scrate = generate_crate_access(&hidden_crate_name, "srml-support");
    let scrate_decl = generate_hidden_includes(&hidden_crate_name, "srml-support", "srml_support");

    let (traitinstance, traittypes) =
        if let GenericParam::Type(syn::TypeParam { ident, bounds, .. }) = strait {
            (ident, bounds)
        } else {
            return try_tok!(Err(Error::new(
                strait.span(),
                "Missing declare store generic params"
            )));
        };
    ...
    let decl_store_items = decl_store_items(&storage_lines);
    let impl_store_items = impl_store_items(&traitinstance, &storage_lines);
    let impl_store_fns = impl_store_fns(&scrate, &traitinstance, &storage_lines);
    let store_functions_to_metadata = store_functions_to_metadata(&scrate, &storage_lines);
...
fn decl_store_extra_genesis(
    ...
    for sline in storage_lines.inner.iter() {
        ...
        let is_simple = if let DeclStorageType::Simple(..) = storage_type {
            true
        } else {
            false
        };
        ...
        if let (Some(ref getter), Some(ref config)) = (getter, config) {
            ...
            let fielddefault = default_value
                .inner
                .get(0)
                .as_ref()
                .map(|d| &d.expr)
        ...
        for ex_content in eg.content.content.lines.inner.iter() {
            match ex_content {
                AddExtraGenesisLineEnum::AddExtraGenesisLine(AddExtraGenesisLine {
                    ...
                    let extra_default = default_value
                        .inner
                        .get(0)
                        .map(|d| &d.expr)
                        .map(|e| quote! { #e })
                ...
                AddExtraGenesisLineEnum::AddExtraGenesisBuild(DeclStorageBuild {
                    expr, ..
                }) => {
                    if has_scall {
...
fn decl_storage_items(
    ...
    for sline in storage_lines.inner.iter() {
        ...
        let (is_simple, extracted_opt, stk, gettype) = match storage_type {
            ...
            DeclStorageType::Map(ref map) => (
                false,
                ext::extract_type_option(&map.value),
                Some(&map.key),
                &map.value,
            ),
        };
...
fn decl_store_items(storage_lines: &ext::Punctuated<DeclStorageLine, Token![;]>) -> TokenStream2 {
    storage_lines.inner.iter().map(|sline| &sline.name).fold(
        TokenStream2::new(),
        |mut items, name| {
