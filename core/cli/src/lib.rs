...
use network::{
    config::{NetworkConfiguration, NonReservedPeerMode},
    multiaddr, Protocol,
};
use primitives::H256;
...
pub use params::{CoreCommands, CoreParams, ExecutionStrategy};
use regex::Regex;
use std::fs;
use std::fs::File;
...
fn get_chain_key(matches: &clap::ArgMatches) -> String {
    matches
        .value_of("chain")
        .unwrap_or_else(|| if matches.is_present("dev") { "dev" } else { "" })
        .into()
}
...
pub fn parse_matches<'a, F, S>(
    ...
    ChainSpec<<F as service::ServiceFactory>::Genesis>,
    ...
    S: FnOnce(&str) -> Result<Option<ChainSpec<FactoryGenesis<F>>>, String>,
{
    ...
    match is_node_name_valid(&config.name) {
        Ok(_) => (),
        Err(msg) => bail!(create_input_err(format!(
    ...
    config.database_cache_size = match matches.value_of("database_cache_size") {
        Some(s) => Some(
            s.parse()
                .map_err(|_| "Invalid Database Cache size specified")?,
        ),
        _ => None,
    };
    ...
    let role = if matches.is_present("light") {
    ...
    config.roles = role;
    {
        ...
        config.network.config_path = Some(
            network_path(&base_path, config.chain_spec.id())
                .to_string_lossy()
                .into(),
        );
        config.network.net_config_path = config.network.config_path.clone();
        config.network.reserved_nodes.extend(
            matches
        ...
        if config.network.listen_addresses.is_empty() {
            ...
            config.network.listen_addresses =
                vec![iter::once(Protocol::Ip4(Ipv4Addr::new(0, 0, 0, 0)))
        ...
        let in_peers = match matches.value_of("in_peers") {
            Some(in_peers) => in_peers
                .parse()
                .map_err(|_| "Invalid in-peers value specified.")?,
            None => 25,
        };
        let out_peers = match matches.value_of("out_peers") {
            Some(out_peers) => out_peers
                .parse()
                .map_err(|_| "Invalid out-peers value specified.")?,
            None => 25,
        };
    ...
    let rpc_interface: &str = if matches.is_present("rpc_external") {
        "0.0.0.0"
    } else {
        "127.0.0.1"
    };
    let ws_interface: &str = if matches.is_present("ws_external") {
        "0.0.0.0"
    } else {
        "127.0.0.1"
    };
...
fn get_db_path_for_subcommand(
    ...
    fn check_contradicting_chain_dev_flags(
        ...
        if m0.is_present("dev") && m1.is_present("chain") {
            bail!(create_input_err(
                "`--dev` and `--chain` given on different levels"
            ));
        }
    ...
    if main_cmd.is_present("base_path") && sub_cmd.is_present("base_path") {
        bail!(create_input_err(
            "`--base_path` option is present two times"
        ));
    }
    ...
    F: ServiceFactory,
{
    ...
    if spec.boot_nodes().is_empty() {
        let network_keys = network::obtain_private_key(config)
            .map_err(|err| format!("Error obtaining network key: {}", err))?;
        let peer_id = network_keys.to_peer_id();
        let addr = multiaddr![Ip4([127, 0, 0, 1]), Tcp(30333u16), P2p(peer_id)];
    ...
    E: Future<Item = (), Error = ()> + Send + 'static,
{
    ...
    Ok(service::chain_ops::export_blocks::<F, _, _>(
        config,
        exit,
        file,
        As::sa(from),
    ...
    E: Future<Item = (), Error = ()> + Send + 'static,
{
        ...
        config.block_execution_strategy = match s {
            ...
            _ => {
                return Err(error::ErrorKind::Input(
                    "Invalid block execution mode specified".to_owned(),
                )
                .into())
            }
        };
    ...
    if let Some(s) = matches.value_of("api-execution") {
        config.api_execution_strategy = match s {
            ...
            _ => {
                return Err(error::ErrorKind::Input(
                    "Invalid API execution mode specified".to_owned(),
                )
                .into())
            }
        };
    ...
    Ok(service::chain_ops::import_blocks::<F, _, _>(
        config, exit, file,
    )?)
}
...
fn purge_chain<F>(db_path: PathBuf) -> error::Result<()>
where
    F: ServiceFactory,
{
