...
use primitives::{convert_hash, AuthorityId, Blake2Hasher, ChangesTrieConfiguration, H256};
use runtime_primitives::traits::{
    As, Block as BlockT, Digest, DigestItem, Header as HeaderT, NumberFor, Zero,
};
use runtime_primitives::BuildStorage;
use runtime_primitives::{generic::BlockId, ChildrenStorageMap, Justification, StorageMap};
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    fn commit_operation(
        ...
        if let Some(pending_block) = operation.pending_block {
            ...
            self.changes_tries_storage
                .commit(&mut transaction, operation.changes_trie_updates);
    ...
    fn finalize_block(
        ...
        if let Some(header) = ::client::blockchain::HeaderBackend::header(&self.blockchain, block)?
        {
            ...
            self.storage.db.write(transaction).map_err(db_err)?;
            self.blockchain
                .update_meta(hash, header.number().clone(), false, true);
            Ok(())
        } else {
        ...
        for c in 0..n.as_() {
            ...
            match self.storage.state_db.revert_one() {
                Some(commit) => {
                    ...
                    self.storage.db.write(transaction).map_err(db_err)?;
                    self.blockchain
                        .update_meta(header.hash().clone(), best.clone(), true, false);
...
mod tests {
    ...
    fn test_finalize_block_with_justification() {
        ...
        backend
            .finalize_block(BlockId::Number(0), justification.clone())
            .unwrap();

        assert_eq!(
            backend
                .blockchain()
                .justification(BlockId::Number(0))
                .unwrap(),
            justification,
        );
    }
