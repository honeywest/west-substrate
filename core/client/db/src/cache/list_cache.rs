...
use cache::list_storage::{Metadata, Storage, StorageTransaction};
...
impl<Block: BlockT, T: CacheItemT, S: Storage<Block, T>> ListCache<Block, T, S> {
    ...
    pub fn value_at_block(&self, at: &ComplexBlockId<Block>) -> ClientResult<Option<T>> {
        ...
        match head {
            Some(head) => head
                .search_best_before(&self.storage, at.number, true)
    ...
    fn prune_finalized_entries<Tx: StorageTransaction<Block, T>>(
        ...
        let mut do_pruning = || -> ClientResult<()> {
            ...
            Fork {
                best_block: None,
                head: Entry {
                    valid_from: first_entry_to_truncate,
                    value: None,
                },
            }
...
mod chain {
    ...
    pub fn is_connected_to_block<Block: BlockT, T: CacheItemT, S: Storage<Block, T>>(
        ...
        while *current.number() > begin.number {
            current = storage.read_header(current.parent_hash())?.ok_or_else(|| {
                ClientErrorKind::UnknownBlock(format!("{}", current.parent_hash()))
            })?;
        }
...
pub mod tests {
    use super::*;
    use cache::list_storage::tests::{DummyStorage, DummyTransaction, FaultyStorage};
    ...
    fn list_value_at_block_works() {
        ...
        // ----------> [100]
        assert_eq!(
            ListCache::<_, u64, _>::new(DummyStorage::new(), 1024, test_id(100))
        ...
        assert_eq!(
            ...
            .value_at_block(&ComplexBlockId::new(2.into(), 100))
            .unwrap(),
            None
        );
    ...
    fn list_on_block_insert_works() {
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, test_id(4), test_id(5), Some(4), false)
                .unwrap(),
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, test_id(4), test_id(5), Some(5), false)
                .unwrap(),
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(4), correct_id(5), Some(4), false)
                .unwrap(),
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(4), correct_id(5), Some(5), false)
                .unwrap(),
            Some(CommitOperation::AppendNewEntry(
                0,
                Entry {
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(3), fork_id(0, 3, 4), Some(14), false)
                .unwrap(),
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(2), false)
                .unwrap(),
            None
        );
        assert!(tx.inserted_entries().is_empty());
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(3), false)
                .unwrap(),
            Some(CommitOperation::AddNewFork(Entry {
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(3), true)
                .unwrap(),
        ...
        assert_eq!(
            cache
                .on_block_insert(&mut tx, correct_id(2), correct_id(3), Some(2), true)
                .unwrap(),
            Some(CommitOperation::BlockFinalized(
                correct_id(3),
                None,
    ...
    fn is_finalized_block_works() {
        ...
        assert_eq!(
            chain::is_finalized_block::<_, u64, _>(
                &DummyStorage::new().with_id(1, From::from(2)),
                &test_id(1),
                100
            )
            .unwrap(),
        ...
        assert_eq!(
            chain::is_finalized_block::<_, u64, _>(
                &DummyStorage::new().with_id(1, From::from(1)),
                &test_id(1),
                100
            )
            .unwrap(),
