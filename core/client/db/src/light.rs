...
use cache::{ComplexBlockId, DbCache, DbCacheSync};
...
use utils::{block_id_to_lookup_key, db_err, meta_keys, open_database, read_db, read_meta, Meta};
...
pub(crate) mod tests {
    ...
    pub fn insert_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, authorities, NewBlockState::Best, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn insert_final_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, authorities, NewBlockState::Final, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn insert_non_best_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, authorities, NewBlockState::Normal, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn finalized_ancient_headers_are_replaced_with_cht() {
        fn insert_headers<F: Fn(&Hash, u64) -> Header>(header_producer: F) -> LightStorage<Block> {
            ...
            let mut prev_hash =
                insert_final_block(&db, None, || header_producer(&Default::default(), 0));
            ...
            for number in 0..cht::SIZE {
                prev_hash = insert_block(&db, None, || {
    ...
    fn get_cht_works() {
        ...
        let mut prev_hash = insert_final_block(&db, None, || {
    ...
    fn authorites_are_cached() {
        ...
        let (hash2, hash6) = {
            ...
            let hash2 = insert_final_block(&db, Some(vec![[1u8; 32].into()]), || {
                default_header(&hash1, 2)
            });
            run_checks(&db, 2, &checks);
            let hash3 = insert_final_block(&db, Some(vec![[1u8; 32].into()]), || {
                default_header(&hash2, 3)
            });
            run_checks(&db, 3, &checks);
            let hash4 =
                insert_final_block(&db, Some(vec![[1u8; 32].into(), [2u8; 32].into()]), || {
...

        {
            ...
            let hash2_1 = insert_non_best_block(&db, Some(vec![[1u8; 32].into()]), || {
                default_header(&hash2, 3)
            });
            assert_eq!(None, db.cache().authorities_at(BlockId::Hash(hash2_1)));
            let hash2_2 =
                insert_non_best_block(&db, Some(vec![[1u8; 32].into(), [2u8; 32].into()]), || {
        ...
        let (hash7, hash8, hash6_1, hash6_2, hash6_1_1, hash6_1_2) = {
            ...
            let hash7 = insert_block(&db, Some(vec![[3u8; 32].into()]), || {
                default_header(&hash6, 7)
            });
            assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6)), None);
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash7)),
                Some(vec![[3u8; 32].into()])
            );
            let hash8 = insert_block(&db, Some(vec![[3u8; 32].into()]), || {
            ...
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash8)),
                Some(vec![[3u8; 32].into()])
            );
            let hash6_1 = insert_block(&db, Some(vec![[4u8; 32].into()]), || {
                default_header(&hash6, 7)
            });
            assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6)), None);
            assert_eq!(
            ...
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash6_1)),
                Some(vec![[4u8; 32].into()])
            );
            let hash6_1_1 = insert_non_best_block(&db, Some(vec![[5u8; 32].into()]), || {
            ...
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash6_1_1)),
                Some(vec![[5u8; 32].into()])
            );
            let hash6_1_2 = insert_non_best_block(&db, Some(vec![[6u8; 32].into()]), || {
                default_header(&hash6_1, 8)
            });
            assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6)), None);
            assert_eq!(
            ...
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash6_1_2)),
                Some(vec![[6u8; 32].into()])
            );
            let hash6_2 = insert_block(&db, Some(vec![[4u8; 32].into()]), || {
                default_header(&hash6_1, 8)
            });
            assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6)), None);
            assert_eq!(
            ...
            assert_eq!(
                db.cache().authorities_at(BlockId::Hash(hash6_2)),
                Some(vec![[4u8; 32].into()])
            );
