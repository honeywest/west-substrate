...
impl<Block: BlockT> StorageNotifications<Block> {
    ...
    pub fn trigger(
        ...
        for subscriber in subscribers {
            let should_remove = {
                let &(ref sink, ref filter) = self.sinks.get(&subscriber).expect(
...
mod tests {
    use super::*;
    use futures::Stream;
    use runtime_primitives::testing::{Block as RawBlock, ExtrinsicWrapper, H256 as Hash};
    ...
    fn should_only_notify_interested_listeners() {
        ...
        assert_eq!(
            recv1.next().unwrap(),
            Ok((1.into(), vec![(StorageKey(vec![1]), None),].into()))
        );
        assert_eq!(
            recv2.next().unwrap(),
            Ok((
                1.into(),
                vec![(StorageKey(vec![2]), Some(StorageData(vec![3]))),].into()
            ))
        );
    }
