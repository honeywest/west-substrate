...
pub mod tests {
    ...
    use test_client::runtime::{self, Block, Hash, Header};
    use test_client::{self, blockchain::HeaderBackend, TestClient};

    use super::*;
    use in_mem::Blockchain as InMemoryBlockchain;
    use light::blockchain::tests::{DummyBlockchain, DummyStorage};
    use light::fetcher::{
    ...
    fn prepare_for_read_proof_check() -> (TestChecker, Header, Vec<Vec<u8>>, usize) {
        ...
        let authorities_len = remote_client
            .authorities_at(&remote_block_id)
            .unwrap()
            .len();
