...
use primitives::{storage::well_known_keys, AuthorityId};
use runtime_primitives::generic::BlockId;
use runtime_primitives::traits::{
    As, Block as BlockT, Digest, DigestItem, Header as HeaderT, NumberFor, Zero,
};
use runtime_primitives::{ChildrenStorageMap, Justification, StorageMap};
use state_machine::backend::{Backend as StateBackend, Consolidate, InMemory};
    ...
    Block::Hash: From<[u8; 32]>,
{
    ...
    fn header_cht_root(
        ...
        self.storage
            ...
            .cloned()
            .ok_or_else(|| {
                error::ErrorKind::Backend(format!("Header CHT for block {} not exists", block))
                    .into()
            })
    }
    ...
    fn changes_trie_cht_root(
        ...
        self.storage
            ...
            .cloned()
            .ok_or_else(|| {
                error::ErrorKind::Backend(format!(
        ...
        if let Some(pending_block) = operation.pending_block {
            ...
            if let Some(changes_trie_root) = changes_trie_root {
                if let Some(changes_trie_update) = operation.changes_trie_update {
                    ...
                    self.changes_trie_storage.insert(
                        header.number().as_(),
                        changes_trie_root,
                        changes_trie_update,
                    );
            ...
            if pending_block.state.is_best() {
                self.blockchain
                    .cache
                    .insert(parent_hash, operation.pending_authorities);
            }
        }
