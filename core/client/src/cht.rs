...
mod tests {
    ...
    fn build_pairs_fails_when_no_enough_blocks() {
        assert!(build_pairs::<Header, _>(
            SIZE,
            0,
            ::std::iter::repeat_with(|| Ok(Some(1.into()))).take(SIZE as usize / 2)
        )
        .is_err());
    }
    ...
    fn build_pairs_fails_when_missing_block() {
        assert!(build_pairs::<Header, _>(
            SIZE,
            0,
            ::std::iter::repeat_with(|| Ok(Some(1.into())))
                .take(SIZE as usize / 2)
                .chain(::std::iter::once(Ok(None)))
                .chain(::std::iter::repeat_with(|| Ok(Some(2.into()))).take(SIZE as usize / 2 - 1))
        )
        .is_err());
    }
    ...
    fn compute_root_works() {
        assert!(compute_root::<Header, Blake2Hasher, _>(
            SIZE,
            42,
            ::std::iter::repeat_with(|| Ok(Some(1.into()))).take(SIZE as usize)
        )
        .is_ok());
    }
