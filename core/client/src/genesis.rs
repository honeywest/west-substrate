...
mod tests {
    ...
    use state_machine::{execute, ExecutionStrategy, InMemoryChangesTrieStorage, OverlayedChanges};
    use test_client;
        ...
        let transactions = txs
            ...
            .map(|tx| {
                let signature = Pair::from(
                    Keyring::from_public(Public::from_raw(tx.from.to_fixed_bytes())).unwrap(),
                )
                .sign(&tx.encode())
                .into();
