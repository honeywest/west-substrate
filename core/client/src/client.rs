...
use codec::Decode;
use consensus::{BlockOrigin, ImportBlock, ImportResult};
use error::Error;
...
use runtime_api::{CallRuntimeAt, ConstructRuntimeApi, Core as CoreAPI};
...
use state_machine::{
    key_changes, key_changes_proof, prove_read, Backend as StateBackend, ChangesTrieAnchorBlockId,
    ChangesTrieRootsStorage, ChangesTrieStorage, CodeExecutor, DBValue, ExecutionManager,
    ExecutionStrategy, OverlayedChanges,
};
use std::{
...
use blockchain::{
    self, Backend as ChainBackend, HeaderBackend as ChainHeaderBackend, Info as ChainInfo,
};
use call_executor::{CallExecutor, LocalCallExecutor};
use executor::{RuntimeInfo, RuntimeVersion};
use light::{call_executor::prove_execution, fetcher::ChangesProof};
use notifications::{StorageEventStream, StorageNotifications};
use {
    block_builder::{self, api::BlockBuilder as BlockBuilderAPI},
    cht, consensus, error, genesis, in_mem,
};
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    pub fn new<S: BuildStorage>(
            ...
            .is_none()
        {
            ...
            info!(
                "Initialising Genesis block/state (state: {}, header-hash: {})",
    ...
    pub fn authorities_at(&self, id: &BlockId<Block>) -> error::Result<Vec<AuthorityId>> {
        match self
            ...
            .blockchain()
            .cache()
            .and_then(|cache| cache.authorities_at(*id))
        {
            Some(cached_value) => Ok(cached_value),
            None => self
                .executor
                .call(id, "Core_authorities", &[])
                .and_then(|r| {
                    Vec::<AuthorityId>::decode(&mut &r.return_data[..])
    ...
    pub fn header_proof_with_cht_size(
        ...
        let proof_error =
            || error::ErrorKind::Backend(format!("Failed to generate header proof for {:?}", id));
    ...
    pub fn key_changes_proof(
        ...
        self.key_changes_proof_with_cht_size(first, last, min, max, key, cht::SIZE)
    }
    ...
    pub fn key_changes_proof_with_cht_size(
        ...
        impl<'a, Block: BlockT> ChangesTrieRootsStorage<Blake2Hasher> for AccessedRootsRecorder<'a, Block> {
            fn root(
                &self,
                anchor: &ChangesTrieAnchorBlockId<H256>,
                block: u64,
    ...
    fn changes_trie_roots_proof_at_cht(
        ...
        let cht_start = cht::start_number(cht_size, cht_num);
        let roots = (cht_start.as_()..).map(|num| {
    ...
    pub fn new_block<InherentData>(
        &self,
    ) -> error::Result<block_builder::BlockBuilder<Block, InherentData, Self>>
    where
        E: Clone + Send + Sync,
    ...
    fn apply_finality(
        ...
        for finalize_new in &enacted[..enacted.len() - 1] {
            self.backend
                .finalize_block(BlockId::Hash(finalize_new.hash), None)?;
        }

        assert_eq!(enacted.last().map(|e| e.hash), Some(block));
        self.backend
            .finalize_block(BlockId::Hash(block), justification)?;
    ...
    pub fn best_containing(
        ...
        let (leaves, best_already_checked) = {
            ...
            let canon_hash = self
                .backend
                .blockchain()
                .hash(*target_header.number())?
                .ok_or_else(|| {
                    error::Error::from(format!(
                        "failed to get hash for block number {}",
    ...
    RA: Send + Sync,
{
    ...
    fn number(
        &self,
        hash: Block::Hash,
    ) -> error::Result<Option<<<Block as BlockT>::Header as HeaderT>::Number>> {
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    fn import_block(
        ...
        match self
            .backend
            .blockchain()
            .status(BlockId::Hash(parent_hash))?
        {
...
impl<B, E, Block, RA> consensus::Authorities<Block> for Client<B, E, Block, RA>
where
    B: backend::Backend<Block, Blake2Hasher>,
...
impl<B, E, Block, RA> CurrentHeight for Client<B, E, Block, RA>
where
    B: backend::Backend<Block, Blake2Hasher>,
    E: CallExecutor<Block, Blake2Hasher> + Clone,
    Block: BlockT<Hash = H256>,
...
impl<B, E, Block, RA> BlockNumberToHash for Client<B, E, Block, RA>
where
    B: backend::Backend<Block, Blake2Hasher>,
    E: CallExecutor<Block, Blake2Hasher> + Clone,
    Block: BlockT<Hash = H256>,
...
pub(crate) mod tests {
    ...
    use test_client::runtime::{self, test_api::TestAPI, Block, RuntimeApi, Transfer};
    ...
    pub fn prepare_client_with_key_changes() -> (
        ...
        let blocks_transfers = vec![
            vec![
                (Keyring::Alice, Keyring::Dave),
                (Keyring::Bob, Keyring::Dave),
            ],
        ...
        // prepare test cases
        let alice = twox_128(&runtime::system::balance_of_key(
            Keyring::Alice.to_raw_public().into(),
        ))
        .to_vec();
        let bob = twox_128(&runtime::system::balance_of_key(
            Keyring::Bob.to_raw_public().into(),
        ))
        .to_vec();
        let charlie = twox_128(&runtime::system::balance_of_key(
        ...
        .to_vec();
        let eve = twox_128(&runtime::system::balance_of_key(
            Keyring::Eve.to_raw_public().into(),
        ))
        .to_vec();
        let ferdie = twox_128(&runtime::system::balance_of_key(
    ...
    fn runtime_api_has_test_api() {
        ...
        assert!(client
            .runtime_api()
            .has_api::<TestAPI<Block>>(&BlockId::Number(client.info().unwrap().chain.best_number),)
            .unwrap());
    }
    ...
    fn authorities_call_works() {
        ...
        assert_eq!(
            client.authorities_at(&BlockId::Number(0)).unwrap(),
            vec![
                Keyring::Alice.to_raw_public().into(),
                Keyring::Bob.to_raw_public().into(),
                Keyring::Charlie.to_raw_public().into()
    ...
    fn block_builder_works_with_transactions() {
        ...
        assert_eq!(client.info().unwrap().chain.best_number, 1);
        assert!(
            client.state_at(&BlockId::Number(1)).unwrap()
                != client.state_at(&BlockId::Number(0)).unwrap()
        );
        assert_eq!(
            client
                .runtime_api()
    ...
    fn client_uses_authorities_from_blockchain_cache() {
        ...
        assert_eq!(
            client
                .authorities_at(&BlockId::Hash(Default::default()))
                .unwrap(),
            vec![[1u8; 32].into()]
        );
    }
    ...
    fn best_containing_with_genesis_block() {
        ...
        assert_eq!(
            genesis_hash.clone(),
            client
                .best_containing(genesis_hash.clone(), None)
                .unwrap()
                .unwrap()
        );
    }
    ...
    fn best_containing_with_hash_not_found() {
        ...
        assert_eq!(
            None,
            client
                .best_containing(uninserted_block.hash().clone(), None)
                .unwrap()
        );
    }
    ...
    fn best_containing_with_single_chain_3_blocks() {
        ...
        assert_eq!(
            a2.hash(),
            client.best_containing(genesis_hash, None).unwrap().unwrap()
        );
        assert_eq!(
            a2.hash(),
            client.best_containing(a1.hash(), None).unwrap().unwrap()
        );
        assert_eq!(
            a2.hash(),
            client.best_containing(a2.hash(), None).unwrap().unwrap()
        );
    }
    ...
    fn best_containing_with_multiple_forks() {
        ...
        // A1 -> A2
        let a2 = client
            .new_block_at(&BlockId::Hash(a1.hash()))
            .unwrap()
            .bake()
            .unwrap();
        client.import(BlockOrigin::Own, a2.clone()).unwrap();
        ...
        // A2 -> A3
        let a3 = client
            .new_block_at(&BlockId::Hash(a2.hash()))
            .unwrap()
            .bake()
            .unwrap();
        ...
        // A3 -> A4
        let a4 = client
            .new_block_at(&BlockId::Hash(a3.hash()))
            .unwrap()
            .bake()
            .unwrap();
        ...
        // A4 -> A5
        let a5 = client
            .new_block_at(&BlockId::Hash(a4.hash()))
            .unwrap()
            .bake()
            .unwrap();
        ...
        // B2 -> B3
        let b3 = client
            .new_block_at(&BlockId::Hash(b2.hash()))
            .unwrap()
            .bake()
            .unwrap();
        ...
        // B3 -> B4
        let b4 = client
            .new_block_at(&BlockId::Hash(b3.hash()))
            .unwrap()
            .bake()
            .unwrap();
        ...
        assert_eq!(
            a5.hash(),
            client.best_containing(a3.hash(), None).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a4.hash(), None).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a5.hash(), None).unwrap().unwrap()
        );

        assert_eq!(
            b4.hash(),
            client.best_containing(b2.hash(), None).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b3.hash(), None).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b4.hash(), None).unwrap().unwrap()
        );

        assert_eq!(
            c3.hash(),
            client.best_containing(c3.hash(), None).unwrap().unwrap()
        );

        assert_eq!(
            d2.hash(),
            client.best_containing(d2.hash(), None).unwrap().unwrap()
        );
        ...
        assert_eq!(
            a5.hash(),
            client
                .best_containing(genesis_hash, Some(5))
                .unwrap()
                .unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a1.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a2.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a3.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a4.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            a5.hash(),
            client.best_containing(a5.hash(), Some(5)).unwrap().unwrap()
        );

        assert_eq!(
            b4.hash(),
            client.best_containing(b2.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b3.hash(), Some(5)).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b4.hash(), Some(5)).unwrap().unwrap()
        );

        assert_eq!(
            c3.hash(),
            client.best_containing(c3.hash(), Some(5)).unwrap().unwrap()
        );

        assert_eq!(
            d2.hash(),
            client.best_containing(d2.hash(), Some(5)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            a4.hash(),
            client
                .best_containing(genesis_hash, Some(4))
                .unwrap()
                .unwrap()
        );
        assert_eq!(
            a4.hash(),
            client.best_containing(a1.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(
            a4.hash(),
            client.best_containing(a2.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(
            a4.hash(),
            client.best_containing(a3.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(
            a4.hash(),
            client.best_containing(a4.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(None, client.best_containing(a5.hash(), Some(4)).unwrap());

        assert_eq!(
            b4.hash(),
            client.best_containing(b2.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b3.hash(), Some(4)).unwrap().unwrap()
        );
        assert_eq!(
            b4.hash(),
            client.best_containing(b4.hash(), Some(4)).unwrap().unwrap()
        );

        assert_eq!(
            c3.hash(),
            client.best_containing(c3.hash(), Some(4)).unwrap().unwrap()
        );

        assert_eq!(
            d2.hash(),
            client.best_containing(d2.hash(), Some(4)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            a3.hash(),
            client
                .best_containing(genesis_hash, Some(3))
                .unwrap()
                .unwrap()
        );
        assert_eq!(
            a3.hash(),
            client.best_containing(a1.hash(), Some(3)).unwrap().unwrap()
        );
        assert_eq!(
            a3.hash(),
            client.best_containing(a2.hash(), Some(3)).unwrap().unwrap()
        );
        assert_eq!(
            a3.hash(),
            client.best_containing(a3.hash(), Some(3)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            b3.hash(),
            client.best_containing(b2.hash(), Some(3)).unwrap().unwrap()
        );
        assert_eq!(
            b3.hash(),
            client.best_containing(b3.hash(), Some(3)).unwrap().unwrap()
        );
        assert_eq!(None, client.best_containing(b4.hash(), Some(3)).unwrap());

        assert_eq!(
            c3.hash(),
            client.best_containing(c3.hash(), Some(3)).unwrap().unwrap()
        );

        assert_eq!(
            d2.hash(),
            client.best_containing(d2.hash(), Some(3)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            a2.hash(),
            client
                .best_containing(genesis_hash, Some(2))
                .unwrap()
                .unwrap()
        );
        assert_eq!(
            a2.hash(),
            client.best_containing(a1.hash(), Some(2)).unwrap().unwrap()
        );
        assert_eq!(
            a2.hash(),
            client.best_containing(a2.hash(), Some(2)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            b2.hash(),
            client.best_containing(b2.hash(), Some(2)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            d2.hash(),
            client.best_containing(d2.hash(), Some(2)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            a1.hash(),
            client
                .best_containing(genesis_hash, Some(1))
                .unwrap()
                .unwrap()
        );
        assert_eq!(
            a1.hash(),
            client.best_containing(a1.hash(), Some(1)).unwrap().unwrap()
        );
        ...
        assert_eq!(
            genesis_hash,
            client
                .best_containing(genesis_hash, Some(0))
                .unwrap()
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            client.best_containing(c3.hash().clone(), Some(0)).unwrap()
        );

        assert_eq!(
            None,
            client.best_containing(d2.hash().clone(), Some(0)).unwrap()
        );
    }
    ...
    fn best_containing_with_max_depth_higher_than_best() {
        ...
        assert_eq!(
            a2.hash(),
            client
                .best_containing(genesis_hash, Some(10))
                .unwrap()
                .unwrap()
        );
    }
    ...
    fn import_with_justification() {
        ...
        assert_eq!(
            client
                .backend()
                .blockchain()
                .justification(BlockId::Hash(a3.hash()))
                .unwrap(),
            Some(justification),
        );
        ...
        assert_eq!(
            client
                .backend()
                .blockchain()
                .justification(BlockId::Hash(a1.hash()))
                .unwrap(),
            None,
        );
        ...
        assert_eq!(
            client
                .backend()
                .blockchain()
                .justification(BlockId::Hash(a2.hash()))
                .unwrap(),
            None,
        );
    }
