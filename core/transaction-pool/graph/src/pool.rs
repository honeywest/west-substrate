...
use std::{collections::HashMap, hash, sync::Arc, time};
...
impl<B: ChainApi> Pool<B> {
                ...
                match self.api.validate_transaction(at, &xt)? {
                    ...
                    TransactionValidity::Unknown => {
                        ...
                        bail!(error::Error::from(
                            error::ErrorKind::UnknownTransactionValidity
                        ))
                    }
                }
            })
    ...
    pub fn prune_tags(
        ...
        {
            let header_hash = self.api.block_id_to_hash(at)?.ok_or_else(|| {
                error::ErrorKind::Msg(format!("Invalid block id: {:?}", at)).into()
            })?;
            let mut listener = self.listener.write();
            for h in hashes {
...
mod tests {
    ...
    fn should_reject_if_temporarily_banned() {
        ...
        // when
        pool.rotator
            .ban(&time::Instant::now(), &[pool.hash_of(&uxt)]);
    ...
    mod listener {
        ...
        fn should_trigger_ready_and_finalised() {
            ...
            // when
            pool.prune_tags(&BlockId::Number(2), vec![vec![0u8]])
                .unwrap();
            assert_eq!(pool.status().ready, 0);
            assert_eq!(pool.status().future, 0);
            ...
            assert_eq!(
                stream.next(),
                Some(Ok(::watcher::Status::Finalised(2.into())))
            );
