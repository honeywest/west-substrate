...
use test_client::runtime::{AccountId, Block, Extrinsic, Hash, Index, Transfer};
...
fn submission_should_work() {
    ...
    pool.submit_one(&BlockId::number(0), uxt(Alice, 209))
        .unwrap();
...
fn multiple_submission_should_work() {
    ...
    pool.submit_one(&BlockId::number(0), uxt(Alice, 209))
        .unwrap();
    pool.submit_one(&BlockId::number(0), uxt(Alice, 210))
        .unwrap();
...
fn early_nonce_should_be_culled() {
    let pool = pool();
    pool.submit_one(&BlockId::number(0), uxt(Alice, 208))
        .unwrap();
...
fn prune_tags_should_work() {
    ...
    pool.prune_tags(&BlockId::number(1), vec![vec![209]])
        .unwrap();
...
fn should_ban_invalid_transactions() {
    ...
    pool.submit_one(&BlockId::number(0), uxt.clone())
        .unwrap_err();
