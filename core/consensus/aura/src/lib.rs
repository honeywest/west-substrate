...
use network::import_queue::{BasicQueue, Verifier};
use primitives::{ed25519, AuthorityId};
use runtime_primitives::traits::{Block, Digest, DigestItemFor, Header, ProvideRuntimeApi};
use runtime_primitives::{generic, generic::BlockId, BasicInherentData, Justification};
    ...
    Error: ::std::error::Error + Send + From<::consensus_common::Error> + 'static,
{
    ...
    ::std::thread::spawn(move || {
        ...
        runtime
            .block_on(on_exit)
            .expect("Exit future should not fail");
    });
}
...
impl<B: Block, C, E, MakeInherent, Inherent> Verifier<B> for AuraVerifier<C, E, MakeInherent>
where
            ...
            CheckedHeader::Checked(pre_header, slot_num, sig) => {
                ...
                if let Some(inner_body) = body.take() {
                    ...
                    let inherent_res = self
                        .client
                        .runtime_api()
                        .check_inherents(&BlockId::Hash(parent_hash), &block, &inherent)
                        .map_err(|e| format!("{:?}", e))?;
...
pub fn import_queue<B, C, E, MakeInherent, Inherent>(
    ...
    extra: E,
    make_inherent: MakeInherent,
) -> AuraImportQueue<B, C, E, MakeInherent>
where
    B: Block,
    C: Authorities<B>
        + BlockImport<B, Error = ::client::error::Error>
        + ProvideRuntimeApi
        + Send
        + Sync,
    C::Api: BlockBuilderApi<B, Inherent>,
...
mod tests {
    ...
    pub struct AuraTestNet {
        peers: Vec<Arc<Peer<AuraVerifier<PeersClient, NothingExtra, InherentProducingFn<()>>, ()>>>,
        started: bool,
    }
