...
use client::runtime_api::{BlockBuilder as BlockBuilderAPI, BlockBuilderError, Core, OldTxQueue};
use client::{CallExecutor, Client as SubstrateClient};
...
use primitives::{ed25519, ed25519::LocalizedSignature, AuthorityId, Blake2Hasher};
use runtime_primitives::generic::{BlockId, BlockOrigin, Era, ImportBlock, ImportResult};
use runtime_primitives::traits::{
    As, Block as BlockT, BlockNumberToHash, Hash as HashT, Header as HeaderT,
};
...
pub use rhododendron::{
    self, AdvanceRoundReason, Communication as RhdCommunication, InputStreamConcluded,
...
pub type LocalizedMessage<B> =
    rhododendron::LocalizedMessage<B, <B as Block>::Hash, AuthorityId, LocalizedSignature>;
...
impl<H: Decode> UncheckedJustification<H> {
    ...
    pub fn decode_justification(justification: Justification) -> Option<Self> {
        let inner: rhododendron::UncheckedJustification<_, _> =
...
impl<H> From<rhododendron::UncheckedJustification<H, LocalizedSignature>>
    for UncheckedJustification<H>
{
...
pub type Communication<B> =
    rhododendron::Communication<B, <B as Block>::Hash, AuthorityId, LocalizedSignature>;
...
pub trait AuthoringApi: Send
    + Sync
    + BlockBuilderAPI<
        <Self as AuthoringApi>::Block,
        InherentData,
        Error = <Self as AuthoringApi>::Error,
    > + Core<
        <Self as AuthoringApi>::Block,
        AuthorityId,
        Error = <Self as AuthoringApi>::Error,
...
pub struct BftFuture<B, P, I, InStream, OutSink>
where
    B: Block + Clone + Eq,
    B::Hash: ::std::hash::Hash,
    P: LocalProposer<B>,
...
impl<B, P, I, InStream, OutSink> Future for BftFuture<B, P, I, InStream, OutSink>
where
    B: Block + Clone + Eq,
    B::Hash: ::std::hash::Hash,
    P: LocalProposer<B>,
    ...
    OutSink: Sink<SinkItem = Communication<B>, SinkError = Error>,
{
    ...
    fn poll(&mut self) -> ::futures::Poll<(), ()> {
        ...
        self.inner
            .context()
            .proposer
            .on_round_end(committed.round_number, true);
        ...
        if let Some(justified_block) = committed.candidate {
            ...
            let just: Justification =
                UncheckedJustification(committed.justification.uncheck()).into();
        ...
        self.inner
            .context()
            .update_round_cache(committed.round_number);

        Ok(Async::Ready(()))
    }
}

impl<B, P, I, InStream, OutSink> Drop for BftFuture<B, P, I, InStream, OutSink>
where
    B: Block + Clone + Eq,
    B::Hash: ::std::hash::Hash,
    P: LocalProposer<B>,
    ...
    OutSink: Sink<SinkItem = Communication<B>, SinkError = Error>,
{
    fn drop(&mut self) {
        ...
        self.inner
            .context()
            .proposer
            .import_misbehavior(misbehavior);
    }
}
    ...
    I: BlockImport<B> + Authorities<B>,
{
    ...
    pub fn build_upon<In, Out>(
        &self,
        header: &B::Header,
        input: In,
        output: Out,
    ) -> Result<Option<BftFuture<B, <P as Environment<B>>::Proposer, I, In, Out>>, P::Error>
    where
        ...
        Out: Sink<SinkItem = Communication<B>, SinkError = Error>,
    {
        ...
        let authorities = self
            .client
            .authorities(&BlockId::Hash(hash.clone()))
        ...
        let mut agreement = rhododendron::agree(bft_instance, n, max_faulty, input, output);
...
where
    S::Error: From<InputStreamConcluded>,
{
                    ...
                    match comms {
                        RhdCommunication::Auxiliary(prepare_just) => {
                            ...
                            if let Ok(checked) = checked {
                                return Ok(Async::Ready(Some(RhdCommunication::Auxiliary(
                                    checked.0,
                                ))));
                            }
                        }
                        RhdCommunication::Consensus(RhdLocalized::Propose(p)) => {
                            if p.sender == self.local_id {
                                continue;
                            }

                            let checked =
                                check_proposal::<B>(&self.authorities, &self.parent_hash, &p);

                            if let Ok(()) = checked {
                                return Ok(Async::Ready(Some(RhdCommunication::Consensus(
                                    RhdLocalized::Propose(p),
                                ))));
                            }
                        }
                        RhdCommunication::Consensus(RhdLocalized::Vote(v)) => {
                            if v.sender == self.local_id {
                                continue;
                            }

                            let checked = check_vote::<B>(&self.authorities, &self.parent_hash, &v);
...
fn check_justification_signed_message<H>(
    ...
    just.0
        .check(
            authorities.len() - max_faulty_of(authorities.len()),
            |_, _, sig| {
                let auth_id = sig.signer.clone().into();
                if !authorities.contains(&auth_id) {
                    return None;
                }
...
pub fn check_justification<B: Block>(
    ...
    let vote: Action<B, B::Hash> =
        Action::Commit(just.0.round_number as u32, just.0.digest.clone());
...
pub fn check_prepare_justification<B: Block>(
    ...
    let vote: Action<B, B::Hash> =
        Action::Prepare(just.0.round_number as u32, just.0.digest.clone());
    let message = localized_encode(parent, vote);

    check_justification_signed_message(authorities, &message[..], just)
        .map(|e| PrepareJustification(e.0))
}
...
fn check_action<B: Block>(
    ...
    if ed25519::verify_strong(&sig.signature, &message, &sig.signer) {
        ...
        Err(
            CommonErrorKind::InvalidSignature(sig.signature.into(), sig.signer.clone().into())
                .into(),
        )
    }
}
...
impl<'a, B, E, Block> BlockBuilder<Block>
    for client::block_builder::BlockBuilder<'a, B, E, Block, Blake2Hasher>
where
...
impl<'a, B, E, Block> AuthoringApi for SubstrateClient<B, E, Block>
where
    B: client::backend::Backend<Block, Blake2Hasher> + Send + Sync + 'static,
    E: CallExecutor<Block, Blake2Hasher> + Send + Sync + Clone + 'static,
    Block: BlockT,
...
/// Proposer factory.
pub struct ProposerFactory<N, C, A>
where
    C: AuthoringApi,
    A: txpool::ChainApi,
{
...
impl<N, C, A> consensus::Environment<<C as AuthoringApi>::Block> for ProposerFactory<N, C, A>
where
    ...
    Error: From<<C as AuthoringApi>::Error>,
{
    ...
    fn init(
        ...
        let random_seed =
            <<<C as AuthoringApi>::Block as BlockT>::Header as HeaderT>::Hashing::hash(
                random_seed.as_ref(),
            );
...
impl<C, A> BaseProposer<<C as AuthoringApi>::Block> for Proposer<C, A>
where
    C: AuthoringApi + BlockNumberToHash,
    A: txpool::ChainApi<Block = <C as AuthoringApi>::Block>,
    <<C as AuthoringApi>::Block as BlockT>::Hash:
    ...
    error::Error: From<<C as AuthoringApi>::Error>,
{
    ...
    fn propose(&self) -> Self::Create {
        ...
        let block =
            self.client
                .build_block(&self.parent_id, inherent_data, |block_builder| {
    ...
    fn evaluate(&self, unchecked_proposal: &<C as AuthoringApi>::Block) -> Self::Evaluate {
        ...
        if let Err(e) =
            evaluation::evaluate_initial(unchecked_proposal, &self.parent_hash, self.parent_number)
        {
        ...
        let proposed_timestamp =
            match self
                .client
                .check_inherents(&self.parent_id, &unchecked_proposal, &inherent)
            {
                Ok(Ok(())) => None,
        ...
        let vote_delays = {
            ...
            let proposed_timestamp =
                ::std::cmp::max(self.minimum_timestamp, proposed_timestamp.unwrap_or(0));
            ...
            match timestamp_delay {
                Some(duration) => {
                    future::Either::A(Delay::new(duration).map_err(|e| ErrorKind::Timer(e).into()))
                }
        ...
        let evaluated = match self
            .client
            .execute_block(&self.parent_id, &unchecked_proposal.clone())
            .map_err(Error::from)
        {
            Ok(()) => Ok(true),
...
impl<C, A> LocalProposer<<C as AuthoringApi>::Block> for Proposer<C, A>
where
    C: AuthoringApi + BlockNumberToHash,
    A: txpool::ChainApi<Block = <C as AuthoringApi>::Block>,
    ...
    error::Error: From<<C as AuthoringApi>::Error>,
{
    ...
    fn import_misbehavior(
        ...
        _misbehavior: Vec<(
            AuthorityId,
            Misbehavior<<<C as AuthoringApi>::Block as BlockT>::Hash>,
        )>,
    ) {
        use node_runtime::{Call, ConsensusCall, UncheckedExtrinsic};
        ...
        for (target, misbehavior) in misbehavior {
                ...
                misbehavior: match misbehavior {
                    ...
                    GenericMisbehavior::DoublePrepare(round, (h1, s1), (h2, s2)) => {
                        MisbehaviorKind::BftDoublePrepare(
                            round as u32,
                            (h1.into(), s1.signature),
                            (h2.into(), s2.signature),
                        )
                    }
            ...
            let extrinsic = UncheckedExtrinsic {
                signature: Some((
                    node_runtime::RawAddress::Id(local_id),
                    signature,
                    payload.0,
                    Era::immortal(),
                )),
            ...
            let uxt: <<C as AuthoringApi>::Block as BlockT>::Extrinsic =
                Decode::decode(&mut extrinsic.encode().as_slice())
...
mod tests {
    ...
    fn make_service(client: FakeClient) -> BftService<TestBlock, DummyFactory, FakeClient> {
        BftService {
    ...
    fn future_gets_preempted() {
        ...
        let mut first_bft = service
            .build_upon(&first, Comms(PhantomData), Comms(PhantomData))
            .unwrap()
            .unwrap();
        assert!(service.live_agreement.lock().as_ref().unwrap().0 == first);

        let _second_bft = service
            .build_upon(&second, Comms(PhantomData), Comms(PhantomData))
            .unwrap();
        assert!(service.live_agreement.lock().as_ref().unwrap().0 != first);
    ...
    fn justification_check_works() {
        ...
        let unchecked = UncheckedJustification(rhododendron::UncheckedJustification {
            digest: hash,
            round_number: 1,
            signatures: authorities_keys
                .iter()
                .take(3)
                .map(|key| sign_vote(rhododendron::Vote::Commit(1, hash).into(), key, parent_hash))
                .collect(),
        });
    ...
    fn bft_can_build_though_skipped() {
        ...
        service
            .build_upon(&third, Comms(PhantomData), Comms(PhantomData))
            .unwrap();
