...
use import_queue::{PassThroughVerifier, SyncImportQueue, Verifier};
use io::SyncIo;
use keyring::Keyring;
...
struct DummyContextExecutor(
    Arc<Protocol<Block, DummySpecialization, Hash>>,
    Arc<RwLock<VecDeque<TestPacket>>>,
);
...
pub type PeersClient = client::Client<
    test_client::Backend,
...
impl<V: 'static + Verifier<Block>, D> Peer<V, D> {
    ...
    fn start(&self) {
        ...
        self.import_queue
            .start(network_link)
            .expect("Test ImportQueue always starts");
    ...
    #[cfg(test)]
    fn with_io<'a, F, U>(&'a self, f: F) -> U
    where
        F: FnOnce(&mut TestIo<'a>) -> U,
    {
    ...
    pub fn gossip_message(&self, topic: Hash, data: Vec<u8>, broadcast: bool) {
        self.sync.gossip_consensus_message(
            &mut TestIo::new(&self.queue, None),
            topic,
            data,
            broadcast,
    ...
    where
        F: FnMut(BlockBuilder<Block, (), PeersClient>) -> Block,
    {
    ...
    pub fn push_blocks(&self, count: usize, with_tx: bool) {
        ...
        if with_tx {
            self.generate_blocks(count, BlockOrigin::File, |mut builder| {
                ...
                let signature = Keyring::from_raw_public(transfer.from.to_fixed_bytes())
                    .unwrap()
    ...
    where
        F: FnOnce(&mut DummySpecialization, &mut Context<Block>) -> U,
    {
...
pub trait TestNetFactory: Sized {
    ...
    fn add_peer(&mut self, config: &ProtocolConfig) {
        ...
        self.mut_peers(|peers| peers.push(peer.clone()));
    }
