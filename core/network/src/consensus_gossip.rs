...
pub use message::generic::{ConsensusMessage, Message};
use network_libp2p::NodeIndex;
use protocol::Context;
use rand::{self, Rng};
use runtime_primitives::generic::BlockId;
use runtime_primitives::traits::{Block as BlockT, Hash, HashFor, Header as HeaderT};
...
impl<B: BlockT> ConsensusGossip<B> {
        ...
        if roles.intersects(Roles::AUTHORITY) {
            ...
            for entry in self.messages.iter() {
                ...
                protocol.send_message(
                    who,
                    Message::Consensus(entry.topic.clone(), entry.message.clone(), entry.broadcast),
                );
            }
            self.peers.insert(
                who,
        ...
        F: Fn() -> ConsensusMessage,
    {
        if broadcast {
            for (id, ref mut peer) in self.peers.iter_mut() {
                if peer
                    .known_messages
                    .insert((topic.clone(), message_hash.clone()))
                {
        ...
        let mut non_authorities: Vec<_> = self
            ...
            .filter_map(|(id, ref peer)| {
                if !peer.is_authority && !peer.known_messages.contains(&(topic, message_hash)) {
                    Some(*id)
                } else {
                    None
                }
    ...
    pub fn messages_for(&mut self, topic: B::Hash) -> mpsc::UnboundedReceiver<ConsensusMessage> {
        ...
        for entry in self.messages.iter().filter(|e| e.topic == topic) {
            tx.unbounded_send(entry.message.clone())
    ...
    pub fn on_incoming(
        ...
        match (
            ...
            protocol.client().header(&BlockId::Hash(topic)),
        ) {
            (_, Err(e)) | (Err(e), _) => {
...
mod tests {
    ...
    fn can_keep_multiple_messages_per_topic() {
        ...
        consensus.register_message(HashFor::<Block>::hash(&msg_a), topic, false, || {
            msg_a.clone()
        });
        consensus.register_message(HashFor::<Block>::hash(&msg_b), topic, false, || {
            msg_b.clone()
        });
