...
use client::{self, BlockStatus, CallExecutor, Client as SubstrateClient, ClientInfo};
use consensus::BlockImport;
use consensus::{ImportBlock, ImportResult};
use primitives::{AuthorityId, Blake2Hasher, H256};
...
pub trait Client<Block: BlockT>: Send + Sync {
    ...
    fn header_proof(
        &self,
        block_number: <Block::Header as HeaderT>::Number,
    ) -> Result<(Block::Header, Vec<Vec<u8>>), Error>;
