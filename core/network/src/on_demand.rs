...
use client::light::fetcher::{
    ChangesProof, FetchChecker, Fetcher, RemoteCallRequest, RemoteChangesRequest,
    RemoteHeaderRequest, RemoteReadRequest,
};
use client::{
    self,
    error::{Error as ClientError, ErrorKind as ClientErrorKind},
};
use config::Roles;
...
enum RequestData<Block: BlockT> {
    RemoteHeader(
        ...
        Sender<Result<Block::Header, ClientError>>,
    ),
    RemoteRead(
        RemoteReadRequest<Block::Header>,
        Sender<Result<Option<Vec<u8>>, ClientError>>,
    ),
    RemoteCall(
        RemoteCallRequest<Block::Header>,
        Sender<Result<client::CallResult, ClientError>>,
    ),
    RemoteChanges(
        RemoteChangesRequest<Block::Header>,
        Sender<Result<Vec<(NumberFor<Block>, u32)>, ClientError>>,
    ),
}
    ...
    fn accept_response<F: FnOnce(Request<B>) -> Accept<B>>(
        ...
        let request = match core.remove(peer, request_id) {
            ...
            None => {
                io.report_peer(
                    peer,
                    Severity::Bad(&format!("Invalid remote {} response from peer", rtype)),
                );
        ...
        let (retry_count, retry_request_data) = match try_accept(request) {
            ...
            Accept::Unexpected(retry_request_data) => {
                io.report_peer(
                    peer,
                    Severity::Bad(&format!(
                        "Unexpected response to remote {} from peer",
                        rtype
                    )),
                );
                core.remove_peer(peer);
    ...
    B::Header: HeaderT,
{
    ...
    fn on_remote_header_response(
        ...
        self.accept_response("header", io, peer, response.id, |request| {
            match request.data {
                RequestData::RemoteHeader(request, sender) => {
                    match self
                        .checker
                        .check_header_proof(&request, response.header, response.proof)
                    {
                        Ok(response) => {
    ...
    fn on_remote_read_response(
        ...
        self.accept_response("read", io, peer, response.id, |request| {
            match request.data {
    ...
    fn on_remote_changes_response(
        ...
        self.accept_response("changes", io, peer, response.id, |request| {
            match request.data {
                RequestData::RemoteChanges(request, sender) => {
                    match self.checker.check_changes_proof(
                        &request,
                        ChangesProof {
...
pub mod tests {
    ...
    impl FetchChecker<Block> for DummyFetchChecker {
        ...
        fn check_execution_proof(
            ...
            _: Vec<Vec<u8>>,
        ) -> ClientResult<client::CallResult> {
            match self.ok {
    ...
    fn receives_remote_failure_after_retry_count_failures() {
        ...
        assert!(!finished
            .wait_for(&mut finished_at, ::std::time::Duration::from_millis(1000))
            .timed_out());
        ...
        let thread = ::std::thread::spawn(move || {
            ...
            assert_eq!(
                ...
                "6443a0b46e0412e626363028115a9f2c\
                 f963eeed526b8b33e5316f08b50d0dc3"
                    .parse()
                    .unwrap()
            );
        });

        on_demand.on_remote_header_response(
            &mut network,
            0,
    ...
    fn receives_remote_changes_response() {
        ...
        on_demand.on_remote_changes_response(
            &mut network,
            0,
