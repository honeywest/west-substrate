...
impl<B: BlockT> ChainSync<B> {
    ...
    pub(crate) fn new_peer(&mut self, protocol: &mut Context<B>, who: NodeIndex) {
        if let Some(info) = protocol.peer_info(who) {
            match (
                block_status(&*protocol.client(), &*self.import_queue, info.best_hash),
                info.best_number,
            ) {
                (Err(e), _) => {
                    ...
                    protocol.report_peer(
                        who,
                        Severity::Useless(&format!(
                            "Error legimimately reading blockchain status: {:?}",
                            e
                        )),
                    );
                }
    ...
    pub(crate) fn on_block_data(
                ...
                PeerSyncState::AncestorSearch(n) => {
                    match response.blocks.get(0) {
                        Some(ref block) => {
                            ...
                            match protocol.client().block_hash(n) {
                                ...
                                Ok(_) => {
                                    ...
                                    protocol.report_peer(
                                        who,
                                        Severity::Bad("Ancestry search: genesis mismatch for peer"),
                                    );
                                    return None;
                                }
                                Err(e) => {
                        ...
                        None => {
                            ...
                            protocol.report_peer(
                                who,
                                Severity::Bad("Invalid response when searching for ancestor"),
                            );
                            return None;
                        }
                    }
        ...
        let best_seen = self.best_seen_block();
        let is_best = new_blocks
            .first()
            .and_then(|b| b.block.header.as_ref())
            .map(|h| best_seen.as_ref().map_or(false, |n| h.number() >= n));
        let origin = if is_best.unwrap_or_default() {
            BlockOrigin::NetworkBroadcast
        } else {
            BlockOrigin::NetworkInitialSync
        };

        if let Some((hash, number)) = new_blocks.last().and_then(|b| {
            b.block
                .header
                .as_ref()
                .map(|h| (b.block.hash.clone(), *h.number()))
        }) {
    ...
    fn is_known_or_already_downloading(&self, protocol: &mut Context<B>, hash: &B::Hash) -> bool {
        self.peers
            ...
            || block_status(&*protocol.client(), &*self.import_queue, *hash)
                .ok()
