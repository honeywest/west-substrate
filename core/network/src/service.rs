...
use error::Error;
use futures::{self, stream, sync::oneshot, Future, Stream};
use import_queue::ImportQueue;
use io::NetSyncIo;
use network_libp2p::{parse_str_addr, Protocol as Libp2pProtocol, RegisteredProtocol};
use network_libp2p::{
    start_service, Service as NetworkService, ServiceEvent as NetworkServiceEvent,
};
use network_libp2p::{ErrorKind, NetworkConfiguration, PeerId, ProtocolId};
use parking_lot::{Mutex, RwLock};
use protocol::{self, Context, Protocol, ProtocolContext, ProtocolStatus};
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>, H: ExHashT> Service<B, S, H> {
    ...
    pub fn on_block_imported(&self, hash: B::Hash, header: &B::Header) {
        self.handler.on_block_imported(
    ...
    pub fn trigger_repropagate(&self) {
        self.handler
            .propagate_extrinsics(&mut NetSyncIo::new(&self.network, self.protocol_id));
    }
    ...
    {
        self.handler
            .with_spec(&mut NetSyncIo::new(&self.network, self.protocol_id), f)
    }
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>, H: ExHashT> ExecuteInContext<B>
    for Service<B, S, H>
{
    fn execute_in_context<F: Fn(&mut ::protocol::Context<B>)>(&self, closure: F) {
        closure(&mut ProtocolContext::new(
            self.handler.context_data(),
            &mut NetSyncIo::new(&self.network, self.protocol_id),
        ))
    }
}

impl<B: BlockT + 'static, S: NetworkSpecialization<B>, H: ExHashT> SyncProvider<B>
    for Service<B, S, H>
{
    /// Get sync status
    fn status(&self) -> ProtocolStatus<B> {
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>, H: ExHashT> ManageNetwork
    for Service<B, S, H>
{
    fn accept_unreserved_peers(&self) {
...
fn run_thread<B: BlockT + 'static, S: NetworkSpecialization<B>, H: ExHashT>(
    ...
    let network = stream::poll_fn(move || network_service2.lock().poll()).for_each(move |event| {
        ...
        match event {
            NetworkServiceEvent::NodeClosed {
                node_index,
            ...
            NetworkServiceEvent::ClosedCustomProtocols {
                node_index,
                protocols,
            } => {
