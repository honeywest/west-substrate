...
pub use consensus::{BlockImport, BlockOrigin, ImportBlock, ImportResult};
...
pub trait ImportQueue<B: BlockT>: Send + Sync {
    ...
    /// begins.
    fn start<L>(&self, _link: L) -> Result<(), Error>
    where
        Self: Sized,
        L: 'static + Link<B>,
    {
...
impl<B: BlockT, V: 'static + Verifier<B>> ImportQueue<B> for BasicQueue<B, V> {
    fn start<L: 'static + Link<B>>(&self, link: L) -> Result<(), Error> {
        debug_assert!(self.handle.lock().is_none());

        let qdata = self.data.clone();
        let verifier = self.verifier.clone();
        let block_import = self.block_import.clone();
        *self.handle.lock() = Some(
            ::std::thread::Builder::new()
                .name("ImportQueue".into())
                .spawn(move || import_thread(block_import, link, qdata, verifier))
    ...
    fn import_blocks(&self, origin: BlockOrigin, blocks: Vec<BlockData<B>>) {
        ...
        let new_best_importing_number = blocks
            .last()
            .and_then(|b| b.block.header.as_ref().map(|h| h.number().clone()))
...
fn import_thread<B: BlockT, L: Link<B>, V: Verifier<B>>(
    ...
    loop {
        ...
        let blocks_hashes: Vec<B::Hash> =
            new_blocks.1.iter().map(|b| b.block.hash.clone()).collect();
...
fn import_many_blocks<'a, B: BlockT, V: Verifier<B>>(
    ...
    for block in blocks {
        ...
        if qdata
            .map(|qdata| qdata.is_stopping.load(Ordering::SeqCst))
            .unwrap_or_default()
        {
            return false;
        }
...
fn process_import_result<B: BlockT>(
    link: &Link<B>,
    result: Result<
        BlockImportResult<B::Hash, <<B as BlockT>::Header as HeaderT>::Number>,
        BlockImportError,
    >,
) -> usize {
    match result {
...
impl<B: BlockT> ImportCB<B> {
    ...
    where
        F: 'static + Fn(BlockOrigin, Vec<BlockData<B>>) -> bool,
    {
...
impl<B: 'static + BlockT, V: 'static + Verifier<B>> ImportQueue<B> for SyncImportQueue<B, V> {
    fn start<L: 'static + Link<B>>(&self, link: L) -> Result<(), Error> {
        ...
        self.link.set(Box::new(move |origin, new_blocks| {
            let verifier = v.clone();
            import_many_blocks(&*import_handle, &link, None, (origin, new_blocks), verifier)
        }));
        Ok(())
    }
    fn clear(&self) {}
...
pub mod tests {
    ...
    fn import_single_good_block_works() {
        ...
        assert_eq!(
            import_single_block(
                ...
                BlockOrigin::File,
                block,
                Arc::new(PassThroughVerifier(true))
            ),
            Ok(BlockImportResult::ImportedUnknown(hash, number))
        );
    }
    ...
    fn process_import_result_works() {
        ...
        assert_eq!(
            process_import_result::<Block>(
                &link,
                Ok(BlockImportResult::ImportedKnown(Default::default(), 0))
            ),
            1
        );
        ...
        assert_eq!(
            process_import_result::<Block>(
                &link,
                Ok(BlockImportResult::ImportedUnknown(Default::default(), 0))
            ),
            1
        );
        assert_eq!(link.total(), 1);
        ...
        let link = TestLink::new();
        assert_eq!(
            process_import_result::<Block>(&link, Err(BlockImportError::IncompleteHeader(Some(0)))),
            0
        );
        assert_eq!(link.total(), 1);
        ...
        let link = TestLink::new();
        assert_eq!(
            process_import_result::<Block>(&link, Err(BlockImportError::UnknownParent)),
            0
        );
        assert_eq!(link.total(), 1);
        ...
        let link = TestLink::new();
        assert_eq!(
            process_import_result::<Block>(&link, Err(BlockImportError::Error)),
            0
        );
        assert_eq!(link.total(), 1);
        assert_eq!(link.restarts.get(), 1);
