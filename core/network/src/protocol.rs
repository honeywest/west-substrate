...
impl<'a, B: BlockT + 'a, H: 'a + ExHashT> ProtocolContext<'a, B, H> {
    pub(crate) fn new(context_data: &'a ContextData<B, H>, io: &'a mut SyncIo) -> Self {
        ProtocolContext { io, context_data }
    }
    ...
    pub fn peer_info(&self, peer: NodeIndex) -> Option<PeerInfo<B>> {
        self.context_data.peers.read().get(&peer).map(|p| PeerInfo {
            roles: p.roles,
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT> Protocol<B, S, H> {
    ...
    pub fn handle_packet(&self, io: &mut SyncIo, who: NodeIndex, mut data: &[u8]) {
        let message: Message<B> = match Decode::decode(&mut data) {
            ...
            None => {
                ...
                io.report_peer(
                    who,
                    Severity::Bad("Peer sent us a packet with invalid format"),
                );
                return;
            }
        };
        ...
        match message {
            ...
            GenericMessage::RemoteCallRequest(request) => {
                self.on_remote_call_request(io, who, request)
            }
            GenericMessage::RemoteCallResponse(response) => {
                self.on_remote_call_response(io, who, response)
            }
            GenericMessage::RemoteReadRequest(request) => {
                self.on_remote_read_request(io, who, request)
            }
            GenericMessage::RemoteReadResponse(response) => {
                self.on_remote_read_response(io, who, response)
            }
            GenericMessage::RemoteHeaderRequest(request) => {
                self.on_remote_header_request(io, who, request)
            }
            GenericMessage::RemoteHeaderResponse(response) => {
                self.on_remote_header_response(io, who, response)
            }
            GenericMessage::RemoteChangesRequest(request) => {
                self.on_remote_changes_request(io, who, request)
            }
            GenericMessage::RemoteChangesResponse(response) => {
                self.on_remote_changes_response(io, who, response)
            }
            GenericMessage::Consensus(topic, msg, broadcast) => {
                self.consensus_gossip.write().on_incoming(
            ...
            other => self.specialization.write().on_message(
                &mut ProtocolContext::new(&self.context_data, io),
                who,
                &mut Some(other),
            ),
    ...
    pub fn on_peer_disconnected(&self, io: &mut SyncIo, peer: NodeIndex) {
        ...
        if removed {
            ...
            self.consensus_gossip
                .write()
                .peer_disconnected(&mut context, peer);
    ...
    fn maintain_peers(&self, io: &mut SyncIo) {
        ...
        self.specialization
            .write()
            .maintain_peers(&mut ProtocolContext::new(&self.context_data, io));
        for p in aborting {
    ...
    fn on_status_message(&self, io: &mut SyncIo, who: NodeIndex, status: message::Status<B>) {
        ...
        self.specialization
            .write()
            .on_connect(&mut context, who, status);
    }
    ...
    pub fn on_block_announce(
        ...
        self.sync.write().on_block_announce(
            &mut ProtocolContext::new(&self.context_data, io),
            who,
            hash,
            &header,
    ...
    fn on_remote_call_response(
        ...
        self.on_demand
            .as_ref()
            .map(|s| s.on_remote_call_response(io, who, response));
    }
    ...
    fn on_remote_read_request(
        ...
        let proof = match self
            .context_data
            .chain
            .read_proof(&request.block, &request.key)
        {
            Ok(proof) => proof,
            Err(error) => {
    ...
    fn on_remote_read_response(
        ...
        self.on_demand
            .as_ref()
            .map(|s| s.on_remote_read_response(io, who, response));
    }
    ...
    fn on_remote_header_response(
        ...
        self.on_demand
            .as_ref()
            .map(|s| s.on_remote_header_response(io, who, response));
    }
