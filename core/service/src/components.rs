...
use client::{
    self,
    runtime_api::{Metadata, TaggedTransactionQueue},
    Client,
};
use client_db;
use config::Configuration;
use network::{self, import_queue::ImportQueue, OnDemand};
...
use {error, maybe_start_server, Service};
...
pub trait ServiceTrait<C: Components>:
    Deref<Target = Service<C>> + Send + Sync + 'static + StartRPC<C> + MaintainTransactionPool<C>
{
}
...
impl<Factory: ServiceFactory> Components for LightComponents<Factory> {
    ...
    > {
        ...
        let fetch_checker = Arc::new(
            client::light::new_fetch_checker::<_, Blake2Hasher, _, _, _>(
