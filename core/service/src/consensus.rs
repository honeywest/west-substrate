...
use client::{self, error, CallExecutor, Client as SubstrateClient};
...
pub trait AuthoringApi: Send + Sync + ProvideRuntimeApi
where
...
impl<'a, B, E, Block, RA> BlockBuilder<Block>
    for client::block_builder::BlockBuilder<
        'a,
        Block,
        BasicInherentData,
        SubstrateClient<B, E, Block, RA>,
    >
where
...
impl<B, E, Block, RA> AuthoringApi for SubstrateClient<B, E, Block, RA>
where
...
impl<C, A, ConsensusData> consensus_common::Environment<<C as AuthoringApi>::Block, ConsensusData>
    for ProposerFactory<C, A>
where
    C: AuthoringApi,
    ...
    Proposer<<C as AuthoringApi>::Block, C, A>:
        consensus_common::Proposer<<C as AuthoringApi>::Block, ConsensusData>,
{
...
impl<Block, C, A> consensus_common::Proposer<<C as AuthoringApi>::Block, AuraConsensusData>
    for Proposer<Block, C, A>
where
    Block: BlockT,
    ...
    client::error::Error: From<<C as AuthoringApi>::Error>,
{
    ...
    fn propose(
        ...
        self.propose_with(ConsensusData {
            timestamp: Some(consensus_data.timestamp),
        })
    }
}

impl<Block, C, A> consensus_common::Proposer<<C as AuthoringApi>::Block, ()>
    for Proposer<Block, C, A>
where
    Block: BlockT,
    C: AuthoringApi<Block = Block>,
...
impl<Block, C, A> Proposer<Block, C, A>
where
    Block: BlockT,
    C: AuthoringApi<Block = Block>,
    <C as ProvideRuntimeApi>::Api: BlockBuilderApi<Block, BasicInherentData>,
