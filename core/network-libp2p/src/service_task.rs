...
use libp2p::kad::{KadConnecController, KadPeer, KadSystem, KadSystemConfig};
...
use {parse_str_addr, Error, ErrorKind, NetworkConfiguration, NodeIndex};
...
where
    TProtos: IntoIterator<Item = RegisteredProtocol>,
{
    ...
    Ok(Service {
        ...
        next_kad_random_query: Interval::new(
...
impl Service {
    ...
    pub fn deny_unreserved_peers(&mut self) -> Vec<NodeIndex> {
        ...
        let to_disconnect: Vec<NodeIndex> = self
            .swarm
            .nodes()
            .filter(|&n| {
                let peer_id = self
    ...
    pub fn ban_node(&mut self, node_index: NodeIndex) {
        ...
        self.drop_node_inner(
            node_index,
            DisconnectReason::Banned,
            Some(PEER_DISABLE_DURATION),
        );
    }
    ...
    fn drop_node_inner(
        ...
        let closed_custom_protocols = self
            .swarm
            .drop_node(node_index)
    ...
    fn num_ingoing_connections(&self) -> usize {
        self.swarm
            .nodes()
            .filter(|&i| {
                self.swarm.node_endpoint(i) == Some(Endpoint::Listener)
                    && !self
                        .reserved_peers
                        .contains(&self.swarm.peer_id_of_node(i).unwrap())
            })
            .count()
    }
    ...
    fn num_outgoing_connections(&self) -> usize {
        self.swarm
            .nodes()
            .filter(|&i| {
                self.swarm.node_endpoint(i) == Some(Endpoint::Dialer)
                    && !self
                        .reserved_peers
                        .contains(&self.swarm.peer_id_of_node(i).unwrap())
            })
    ...
    fn perform_kad_random_query(&mut self) {
        ...
        let stream = self.kad_system.find_node(random_peer_id, move |who| {
    ...
    ///
    /// > **Note**: Must be called from inside `poll()`, otherwise it will panic.
    fn process_network_event(&mut self, event: SwarmEvent) -> Option<ServiceEvent> {
        match event {
            SwarmEvent::NodePending {
                node_index,
                peer_id,
                endpoint,
            } => {
            ...
            SwarmEvent::Reconnected {
                node_index,
                endpoint,
                closed_custom_protocols,
            } => {
                if let Some(ConnectedPoint::Dialer { address }) =
                    self.nodes_addresses.remove(&node_index)
                {
                    self.topology
                        .report_disconnected(&address, DisconnectReason::FoundBetterAddr);
                }
                if let ConnectedPoint::Dialer { ref address } = endpoint {
                    let peer_id = self
                        .swarm
                        .peer_id_of_node(node_index)
            ...
            SwarmEvent::NodeClosed {
                ...
                if let Some(ConnectedPoint::Dialer { ref address }) =
                    self.nodes_addresses.get(&node_index)
                {
                    self.topology
                        .report_disconnected(address, DisconnectReason::RemoteClosed);
                }
                self.connect_to_nodes();
            ...
            SwarmEvent::UnresponsiveNode { node_index } => {
                ...
                {
                    self.topology
                        .report_disconnected(&address, DisconnectReason::Useless);
                }
                Some(ServiceEvent::NodeClosed {
                    node_index,
                    closed_custom_protocols,
            ...
            SwarmEvent::NodeInfos {
                ...
                let peer_id = self
                    ...
                    .expect("the swarm always produces events containing valid node indices");
                self.topology
                    .add_self_reported_listen_addrs(peer_id, listen_addrs.into_iter());
                None
            }
            SwarmEvent::KadFindNode {
                searched,
                responder,
                ..
            } => {
            ...
            SwarmEvent::KadClosed { .. } => None,
            SwarmEvent::OpenedCustomProtocol {
                node_index,
                protocol,
                version,
            } => {
            ...
            SwarmEvent::ClosedCustomProtocol {
                node_index,
                protocol,
