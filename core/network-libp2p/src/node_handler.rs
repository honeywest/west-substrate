...
use bytes::Bytes;
use custom_proto::{RegisteredProtocolSubstream, RegisteredProtocols};
use futures::{prelude::*, task};
use libp2p::core::nodes::handled_node::{NodeHandler, NodeHandlerEndpoint, NodeHandlerEvent};
use libp2p::core::{upgrade, ConnectionUpgrade, Endpoint, PeerId, PublicKey};
use libp2p::kad::{KadConnecConfig, KadConnecController, KadFindNodeRespond, KadIncomingRequest};
...
pub struct SubstrateNodeHandler<TSubstream> {
    ...
    upgrades_in_progress_listen:
        Vec<Box<Future<Item = FinalUpgrade<TSubstream>, Error = IoError> + Send>>,
    ...
    upgrades_in_progress_dial: Vec<(
        UpgradePurpose,
        Box<Future<Item = FinalUpgrade<TSubstream>, Error = IoError> + Send>,
    )>,
...
pub enum SubstrateInEvent {
    ...
    SendCustomMessage { protocol: ProtocolId, data: Vec<u8> },
...
macro_rules! listener_upgrade {
    ($self:expr) => {
        upgrade::or(
            upgrade::or(
                upgrade::or(
                    upgrade::map((*$self.registered_custom).clone(), move |c| {
                        FinalUpgrade::Custom(c)
                    }),
                    upgrade::map(KadConnecConfig::new(), move |(c, s)| {
                        FinalUpgrade::Kad(c, s)
                    }),
                ),
                upgrade::map(ping::protocol::Ping::default(), move |p| {
                    FinalUpgrade::from(p)
                }),
            ),
            upgrade::map(identify::IdentifyProtocolConfig, move |i| {
                FinalUpgrade::from(i)
            }),
        )
    ...
    TSubstream: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    pub fn new(registered_custom: Arc<RegisteredProtocols>) -> Self {
        ...
        let queued_dial_upgrades = registered_custom
            .0
            .iter()
    ...
    TSubstream: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    fn inject_substream(
        ...
        if endpoint == NodeHandlerEndpoint::Listener {
            ...
            self.upgrades_in_progress_listen
                .push(Box::new(upgrade) as Box<_>);
        ...
        match purpose {
            UpgradePurpose::Custom(id) => {
                ...
                self.upgrades_in_progress_dial
                    .push((purpose, Box::new(upgrade) as Box<_>));
            }
            UpgradePurpose::Kad => {
                let wanted = upgrade::map(KadConnecConfig::new(), move |(c, s)| {
                    FinalUpgrade::Kad(c, s)
                });
                let upgrade = upgrade::apply(substream, wanted, Endpoint::Dialer);
                self.upgrades_in_progress_dial
            ...
            UpgradePurpose::Identify => {
                let wanted = upgrade::map(identify::IdentifyProtocolConfig, move |i| {
                    FinalUpgrade::from(i)
                });
                let upgrade = upgrade::apply(substream, wanted, Endpoint::Dialer);
                self.upgrades_in_progress_dial
            ...
            UpgradePurpose::Ping => {
                let wanted = upgrade::map(ping::protocol::Ping::default(), move |p| {
                    FinalUpgrade::from(p)
                });
                let upgrade = upgrade::apply(substream, wanted, Endpoint::Dialer);
                self.upgrades_in_progress_dial
    ...
    fn poll(
        &mut self,
    ) -> Poll<Option<NodeHandlerEvent<Self::OutboundOpenInfo, Self::OutEvent>>, IoError> {
    ...
    TSubstream: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    fn send_custom_message(&mut self, protocol: ProtocolId, data: Vec<u8>) {
        debug_assert!(
            self.registered_custom.has_protocol(protocol),
            "invalid protocol id requested in the API of the libp2p networking"
        );
        let proto = match self
            .custom_protocols_substreams
            .iter_mut()
            .find(|p| p.protocol_id() == protocol)
        {
            Some(proto) => proto,
            None => {
    ...
    fn inject_fully_negotiated(
        ...
        match upgrade {
            FinalUpgrade::IdentifyListener(sender) => Some(
                SubstrateOutEvent::IdentificationRequest(IdentificationRequest {
                    sender,
            ...
            FinalUpgrade::IdentifyDialer(info, observed_addr) => {
                ...
                Some(SubstrateOutEvent::Identified {
                    info,
                    observed_addr,
                })
            }
            ...
            FinalUpgrade::Custom(proto) => {
                ...
                if self
                    .custom_protocols_substreams
                    .iter()
                    .any(|p| p.protocol_id() == proto.protocol_id())
                {
    ...
    fn poll_upgrades_in_progress(
        &mut self,
    ) -> Poll<Option<SubstrateOutEvent<TSubstream>>, IoError> {
        ...
        for n in (0..self.upgrades_in_progress_listen.len()).rev() {
            ...
            match in_progress.poll() {
                ...
                Err(err) => {
                    return Ok(Async::Ready(Some(SubstrateOutEvent::SubstreamUpgradeFail(
                        err,
                    ))));
                }
            }
            ...
            match custom_proto.poll() {
                ...
                Ok(Async::Ready(None)) => {
                    ...
                    self.queued_dial_upgrades
                        .push(UpgradePurpose::Custom(custom_proto.protocol_id()));
                    self.num_out_user_must_open += 1;
                    return Ok(Async::Ready(Some(
                        SubstrateOutEvent::CustomProtocolClosed {
    ...
    fn poll_kademlia(&mut self) -> Poll<Option<SubstrateOutEvent<TSubstream>>, IoError> {
        ...
        if self.need_report_kad_open {
            ...
            if let Some((ref kad_ctrl, _)) = self.kademlia_substream {
                return Ok(Async::Ready(Some(SubstrateOutEvent::KadOpen(
                    kad_ctrl.clone(),
                ))));
            }
        }
        ...
        if let Some((controller, mut stream)) = self.kademlia_substream.take() {
            loop {
                match stream.poll() {
                    Ok(Async::Ready(Some(KadIncomingRequest::FindNode {
                        searched,
                        responder,
                    }))) => {
                        self.kademlia_substream = Some((controller, stream));
                        return Ok(Async::Ready(Some(SubstrateOutEvent::KadFindNode {
                            searched,
                            responder,
                        })));
                    }
                    ...
                    Ok(Async::Ready(None)) => {
                        return Ok(Async::Ready(Some(SubstrateOutEvent::KadClosed(Ok(())))))
                    }
                    Err(err) => {
                        return Ok(Async::Ready(Some(SubstrateOutEvent::KadClosed(Err(err)))))
                    }
                }
            }
        }
    ...
    fn poll_ping(&mut self) -> Poll<Option<SubstrateOutEvent<TSubstream>>, IoError> {
        ...
        match self.next_ping.poll() {
            ...
            Ok(Async::Ready(())) => {
                ...
                self.next_ping
                    .reset(Instant::now() + Duration::from_secs(5 * 60));
                if self.ping_remote() {
        ...
        for n in (0..self.ping_in_substreams.len()).rev() {
            ...
            match ping.poll() {
                ...
                Err(err) => {
                    warn!(target: "sub-libp2p", "Remote ping substream errored:  {:?}", err)
                }
            }
        }
        ...
        if let Some(mut ping_dialer) = self.ping_out_substream.take() {
            match ping_dialer.poll() {
                Ok(Async::Ready(Some(started))) => {
                    ...
                    return Ok(Async::Ready(Some(SubstrateOutEvent::PingSuccess(
                        started.elapsed(),
                    ))));
                }
                Ok(Async::Ready(None)) => {
    ...
    fn poll_identify(&mut self) -> Poll<Option<SubstrateOutEvent<TSubstream>>, IoError> {
        ...
        for n in (0..identify_send_back.len()).rev() {
            ...
            match id_send_back.poll() {
                ...
                Err(err) => {
                    warn!(target: "sub-libp2p", "Sending back identify info errored: {:?}", err)
                }
            }
        }
...
impl<TSubstream> From<ping::protocol::PingOutput<TSubstream, Instant>>
    for FinalUpgrade<TSubstream>
{
