...
impl NetTopology {
    ...
    pub fn cleanup(&mut self) {
        ...
        self.store.retain(|_, peer| {
            peer.addrs
                .retain(|a| a.expires > now_systime || a.is_connected());
            !peer.addrs.is_empty()
        });
    }
    ...
    pub fn addrs_of_peer(&self, peer: &PeerId) -> impl Iterator<Item = (&Multiaddr, bool)> {
        ...
        let mut list = peer
            ...
            .filter_map(move |addr| {
                ...
                if (addr.expires >= now_st && score > 0 && addr.back_off_until < now_is)
                    || connected
                {
    ...
    /// method was a no-op.
    #[inline]
    pub fn add_self_reported_listen_addrs<I>(&mut self, peer_id: &PeerId, addrs: I) -> bool
    where
    ...
    /// method was a no-op.
    #[inline]
    pub fn add_kademlia_discovered_addrs<I>(&mut self, peer_id: &PeerId, addrs: I) -> bool
    where
        I: Iterator<Item = (Multiaddr, bool)>,
    {
    ...
    /// Returns `true` if the topology has changed in some way. Returns `false` if calling this
    /// method was a no-op.
    fn add_discovered_addrs<I>(&mut self, peer_id: &PeerId, addrs: I) -> bool
    where
        I: Iterator<Item = (Multiaddr, bool)>,
    {
        ...
        'addrs_inserter: for (addr, connectable) in addrs {
            ...
            while peer.addrs.len() >= MAX_ADDRESSES_PER_PEER {
                let pos = peer
                    .addrs
                    .iter()
                    .position(|addr| addr.score() <= initial_score);
                if let Some(pos) = pos {
        ...
        for info in self.store.values_mut() {
            for a in info.addrs.iter_mut() {
                if &a.addr == addr {
                    ...
                    a.next_back_off =
                        cmp::min(a.next_back_off * FAIL_BACKOFF_MULTIPLIER, MAX_BACKOFF);
...
impl Addr {
    ...
    fn flush(score: &mut AddrScore, now: Instant) {
        if let Some(connected_since) = score.connected_since {
            let potential_score: u32 =
                div_dur_with_dur(now - connected_since, CONNEC_DURATION_PER_SCORE);
            // We flush when we connect to an address.
            debug_assert!(score.latest_score_update >= connected_since);
            let effective_score: u32 = div_dur_with_dur(
                score.latest_score_update - connected_since,
                CONNEC_DURATION_PER_SCORE,
            );
...
fn serialize<W: Write>(out: W, map: &FnvHashMap<PeerId, PeerInfo>) -> Result<(), IoError> {
    ...
    serde_json::to_writer_pretty(out, &array).map_err(|err| IoError::new(IoErrorKind::Other, err))
}
