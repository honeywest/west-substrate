...
use libp2p::core::nodes::{ConnectedPoint, Peer as SwarmPeer, RawSwarm, RawSwarmEvent, Substream};
...
use node_handler::{
    IdentificationRequest, SubstrateInEvent, SubstrateNodeHandler, SubstrateOutEvent,
};
...
impl Swarm {
    ...
    pub fn ensure_connection(&mut self, peer_id: PeerId, addr: Multiaddr) -> Result<bool, ()> {
        match self.swarm.peer(peer_id.clone()) {
            ...
            SwarmPeer::NotConnected(peer) => {
                ...
                match peer.connect(
                    addr,
                    SubstrateNodeHandler::new(self.registered_custom.clone()),
                ) {
                    Ok(_) => Ok(false),
                    Err(_) => Err(()),
                }
    ...
    fn handle_node_event(
        ...
        match event {
            ...
            SubstrateOutEvent::KadFindNode {
                searched,
                responder,
            } => Some(SwarmEvent::KadFindNode {
                node_index,
                searched,
                responder,
            }),
            ...
            SubstrateOutEvent::CustomProtocolOpen {
                protocol_id,
                version,
            } => {
        ...
        loop {
            let (peer_id, node_event) = match self.swarm.poll() {
                ...
                Async::Ready(RawSwarmEvent::Replaced {
                    ...
                    let closed_custom_protocols =
                        mem::replace(&mut infos.open_protocols, Vec::new());
