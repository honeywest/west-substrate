...
use utils::{
    fold_fn_decl_for_client_side, generate_crate_access, generate_hidden_includes,
    generate_runtime_mod_name_for_trait, unwrap_or_error,
};
...
use syn::{
    ...
    spanned::Spanned,
    visit::{self, Visit},
    Attribute, FnArg, FnDecl, GenericParam, Generics, ItemTrait, Lit, Meta, NestedMeta, Pat,
    TraitBound, Type,
};
...
impl<'a> Fold for ToClientSideDecl<'a> {
    fn fold_fn_decl(&mut self, input: FnDecl) -> FnDecl {
        let input = fold_fn_decl_for_client_side(input, &self.block_id, &self.crate_);
    ...
    fn fold_item_trait(&mut self, mut input: ItemTrait) -> ItemTrait {
        ...
        *self.found_attributes = remove_supported_attributes(&mut input.attrs);
        ...
        if is_core_trait {
            ...
            input
                .supertraits
                .push(parse_quote!( #crate_::runtime_api::Core<Block> ));
        }
...
impl<'ast> Visit<'ast> for CheckTraitDecl {
    fn visit_fn_arg(&mut self, input: &'ast FnArg) {
        match input {
            FnArg::Captured(ref arg) => {
                ...
                match arg.ty {
                    Type::Reference(ref reference) => self.errors.push(Error::new(
                        reference.span(),
    ...
    fn visit_generic_param(&mut self, input: &'ast GenericParam) {
        match input {
            GenericParam::Type(ty) if &ty.ident == "Block" => self.errors.push(Error::new(
                input.span(),
