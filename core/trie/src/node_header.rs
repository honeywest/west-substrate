...
use super::{
    BRANCH_NODE_NO_VALUE, BRANCH_NODE_WITH_VALUE, EMPTY_TRIE, EXTENSION_NODE_BIG,
    EXTENSION_NODE_OFFSET, EXTENSION_NODE_SMALL_MAX, EXTENSION_NODE_THRESHOLD, LEAF_NODE_BIG,
    LEAF_NODE_OFFSET, LEAF_NODE_SMALL_MAX, LEAF_NODE_THRESHOLD,
};
    ...
    fn encode_to<T: Output>(&self, output: &mut T) {
        match self {
            ...
            NodeHeader::Extension(nibble_count)
                if *nibble_count < EXTENSION_NODE_THRESHOLD as usize =>
            {
...
impl Decode for NodeHeader {
    fn decode<I: Input>(input: &mut I) -> Option<Self> {
        Some(match input.read_byte()? {
            ...
            // 253
            {
                NodeHeader::Extension(
                    input.read_byte()? as usize + EXTENSION_NODE_THRESHOLD as usize,
                )
            }
