...
use super::{
    branch_node, EMPTY_TRIE, EXTENSION_NODE_BIG, EXTENSION_NODE_OFFSET, LEAF_NODE_BIG,
    LEAF_NODE_OFFSET,
};
...
fn fuse_nibbles_node<'a>(nibbles: &'a [u8], leaf: bool) -> impl Iterator<Item = u8> + 'a {
    ...
    let (first_byte_small, big_threshold) = if leaf {
        (
            LEAF_NODE_OFFSET,
            (LEAF_NODE_BIG - LEAF_NODE_OFFSET) as usize,
        )
    } else {
        (
            EXTENSION_NODE_OFFSET,
            (EXTENSION_NODE_BIG - EXTENSION_NODE_OFFSET) as usize,
        )
    };
    let first_byte = first_byte_small + nibbles.len().min(big_threshold) as u8;
    once(first_byte)
        .chain(if nibbles.len() >= big_threshold {
            Some((nibbles.len() - big_threshold) as u8)
        } else {
            None
        })
