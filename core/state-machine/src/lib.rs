...
pub use changes_trie::{
    key_changes, key_changes_proof, key_changes_proof_check, prune as prune_changes_tries,
    AnchorBlockId as ChangesTrieAnchorBlockId, InMemoryStorage as InMemoryChangesTrieStorage,
    RootsStorage as ChangesTrieRootsStorage, Storage as ChangesTrieStorage,
};
pub use ext::Ext;
pub use overlayed_changes::OverlayedChanges;
pub use proving_backend::{create_proof_check_backend, create_proof_check_backend_storage};
pub use testing::TestExternalities;
pub use trie::{DBValue, MemoryDB, TrieDBMut, TrieMut};
...
) -> ExecutionManager<fn(Result<Vec<u8>, E>, Result<Vec<u8>, E>) -> Result<Vec<u8>, E>> {
    ...
    ) -> Result<Vec<u8>, Exec::Error>,
{
    ...
    let heap_pages = try_read_overlay_value(overlay, backend, well_known_keys::HEAP_PAGES)?
        ...
        .unwrap_or(DEFAULT_HEAP_PAGES) as usize;
    ...
    let result = {
        ...
        let (result, storage_delta, changes_delta) = if let (
            true,
        ...
        ) = (was_native, manager)
        {
            ...
            if (result.is_ok()
                && wasm_result.is_ok()
                && result.as_ref().unwrap() == wasm_result.as_ref().unwrap()/* && delta == wasm_delta*/)
...
pub(crate) fn set_changes_trie_config(
    ...
    let config = match config {
        Some(v) => Some(
            changes_trie::Configuration::decode(&mut &v[..]).ok_or_else(|| {
                Box::new("Failed to decode changes trie configuration".to_owned()) as Box<Error>
            })?,
        ),
        None => None,
    };
...
mod tests {
    ...
    fn set_child_storage_works() {
        let backend = InMemory::<Blake2Hasher>::default()
            .try_into_trie_backend()
            .unwrap();
        let changes_trie_storage = InMemoryChangesTrieStorage::new();
        ...
        assert!(ext.set_child_storage(
            b":child_storage:testchild".to_vec(),
            b"abc".to_vec(),
            b"def".to_vec()
        ));
        assert_eq!(
            ext.child_storage(b":child_storage:testchild", b"abc"),
            Some(b"def".to_vec())
        );
    ...
    fn prove_read_and_proof_check_works() {
        ...
        // check proof locally
        let local_result1 =
            read_proof_check::<Blake2Hasher>(remote_root, remote_proof.clone(), b"value2").unwrap();
        let local_result2 =
            read_proof_check::<Blake2Hasher>(remote_root, remote_proof.clone(), &[0xff]).is_ok();
