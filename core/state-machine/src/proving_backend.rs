    ...
    H::Out: HeapSizeOf,
{
    pub fn storage(&mut self, key: &[u8]) -> Result<Option<Vec<u8>>, String> {
        ...
        read_trie_value_with(&eph, self.backend.root(), key, &mut *self.proof_recorder)
            .map_err(map_e)
    }
    ...
    pub fn child_storage(
        ...
        read_child_trie_value_with(storage_key, &eph, &root, key, &mut *self.proof_recorder)
            .map_err(map_e)
    }
...
pub fn create_proof_check_backend_storage<H>(proof: Vec<Vec<u8>>) -> MemoryDB<H>
where
    H: Hasher,
    H::Out: HeapSizeOf,
{
...
mod tests {
    ...
    fn test_proving<'a>(
        trie_backend: &'a TrieBackend<MemoryDB<Blake2Hasher>, Blake2Hasher>,
    ) -> ProvingBackend<'a, MemoryDB<Blake2Hasher>, Blake2Hasher> {
    ...
    fn proof_is_invalid_when_does_not_contains_root() {
        use primitives::H256;
        assert!(
            create_proof_check_backend::<Blake2Hasher>(H256::from_low_u64_be(1), vec![]).is_err()
        );
    }
