...
use changes_trie::input::{DigestIndex, DigestIndexValue, ExtrinsicIndex, ExtrinsicIndexValue};
use changes_trie::storage::{InMemoryStorage, TrieBackendAdapter};
...
use substrate_trie::{MemoryDB, Recorder};
        ...
        if current < self.digest_step {
            ...
            if next == 0 || next < self.begin {
                ...
                let (current, current_begin, digest_step, digest_level) =
                    match lower_bound_max_digest(self.config, self.max, self.begin, next) {
...
impl<'a, RS: 'a + RootsStorage<H>, S: Storage<H>, H: Hasher>
    DrilldownIteratorEssence<'a, RS, S, H>
{
            ...
            if let Some((block, level)) = self.blocks.pop_front() {
                ...
                if block <= self.end.number {
                    ...
                    if let Some(extrinsics) = extrinsics? {
                        let extrinsics: Option<ExtrinsicIndexValue> =
                            Decode::decode(&mut &extrinsics[..]);
                        if let Some(extrinsics) = extrinsics {
                            self.extrinsics
                                .extend(extrinsics.into_iter().rev().map(|e| (block, e)));
                        }
                    }
                }
...
impl<'a, RS: 'a + RootsStorage<H>, S: Storage<H>, H: Hasher>
    ProvingDrilldownIterator<'a, RS, S, H>
{
...
impl<'a, RS: 'a + RootsStorage<H>, S: Storage<H>, H: Hasher> Iterator
    for ProvingDrilldownIterator<'a, RS, S, H>
where
    H::Out: HeapSizeOf,
{
    type Item = Result<(u64, u32), String>;
...
fn surface_iterator<'a>(
    config: &'a Configuration,
    max: u64,
    begin: u64,
    end: u64,
) -> Result<SurfaceIterator<'a>, String> {
    let (current, current_begin, digest_step, digest_level) =
        lower_bound_max_digest(config, max, begin, end)?;
    Ok(SurfaceIterator {
        config,
        begin,
        max,
...
fn lower_bound_max_digest(
    ...
    Ok((current, current_begin, digest_step, digest_level))
}
