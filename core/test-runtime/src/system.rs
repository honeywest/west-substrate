...
use super::{AccountId, Block, BlockNumber, Digest, Extrinsic, Header, H256 as Hash};
use codec::{Encode, KeyedVec};
use primitives::storage::well_known_keys;
use primitives::Blake2Hasher;
use rstd::prelude::*;
use runtime_io::{enumerated_trie_root, storage_changes_root, storage_root, twox_128};
use runtime_primitives::generic;
use runtime_primitives::traits::{BlakeTwo256, Digest as DigestT, Hash as HashT};
use runtime_primitives::{
    transaction_validity::TransactionValidity, ApplyError, ApplyOutcome, ApplyResult,
};
...
pub fn authorities() -> Vec<::primitives::AuthorityId> {
    ...
    (0..len)
        .map(|i| {
            storage::unhashed::get(&i.to_keyed_vec(well_known_keys::AUTHORITY_PREFIX))
...
pub fn execute_block(block: Block) {
    ...
    block.extrinsics.iter().enumerate().for_each(|(i, e)| {
        ...
        execute_transaction_backend(e)
            .map_err(|_| ())
            .expect("Extrinsic error");
    ...
    if let Some(storage_changes_root) =
        storage_changes_root(header.parent_hash.into(), header.number - 1)
    {
        digest.push(generic::DigestItem::ChangesTrieRoot::<Hash, u64>(
            storage_changes_root.into(),
        ));
    }
    assert!(
        digest == header.digest,
        "Header digest items must match that calculated."
    );
}
...
pub fn validate_transaction(utx: Extrinsic) -> TransactionValidity {
    ...
    let hash =
        |from: &AccountId, nonce: u64| twox_128(&nonce.to_keyed_vec(from.as_bytes())).to_vec();
...
mod tests {
    ...
    fn construct_signed_tx(tx: Transfer) -> Extrinsic {
        let signature = Keyring::from_raw_public(tx.from.to_fixed_bytes())
            .unwrap()
            .sign(&tx.encode())
            .into();
        Extrinsic {
    ...
    fn block_import_works() {
        ...
        let h = Header {
            ...
            number: 1,
            state_root: hex!("3d6f3663e052a7d325d3ac6cdbd3cd4033132f5bfe5852d51d4e42e7021ee69b")
                .into(),
            extrinsics_root: hex!(
    ...
    fn block_import_with_transaction_works() {
        ...
        let b = Block {
            header: Header {
                ...
                number: 1,
                state_root: hex!(
                    "c3d2cc317b5897af4c7f65d76b028971ce9fad745678732ff6d42301b4245a9c"
                )
                .into(),
                extrinsics_root: hex!(
                    "4e689a607609f69df099af82577ae6c5969c44f1afe33a43cd7af926eba42272"
                )
                .into(),
                digest: Digest { logs: vec![] },
            },
        ...
        let b = Block {
            header: Header {
                ...
                number: 2,
                state_root: hex!(
                    "2c822d948bb68d7f7a1976d4f827a276a95a3ba1c4c15dbfab3bafbeb85f2b4d"
                )
                .into(),
                extrinsics_root: hex!(
                    "009268a854b21f339c53d3c7a6619a27f564703311d91f11f61573a7fed5ca1c"
                )
                .into(),
                digest: Digest { logs: vec![] },
            },
