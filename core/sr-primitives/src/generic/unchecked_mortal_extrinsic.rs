...
mod tests {
    ...
    fn signed_codec_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, DUMMY_FUNCTION, Era::immortal(), 0u64).encode(),
            ),
            Era::immortal(),
        );
        let encoded = ux.encode();
    ...
    fn mortal_signed_check_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, DUMMY_FUNCTION, Era::mortal(32, 42), 42u64).encode(),
            ),
            Era::mortal(32, 42),
        );
        assert!(ux.is_signed().unwrap_or(false));
    ...
    fn later_mortal_signed_check_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, DUMMY_FUNCTION, Era::mortal(32, 11), 11u64).encode(),
            ),
            Era::mortal(32, 11),
        );
        assert!(ux.is_signed().unwrap_or(false));
    ...
    fn too_late_mortal_signed_check_should_fail() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, DUMMY_FUNCTION, Era::mortal(32, 10), 10u64).encode(),
            ),
            Era::mortal(32, 10),
        );
        assert!(ux.is_signed().unwrap_or(false));
    ...
    fn too_early_mortal_signed_check_should_fail() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, DUMMY_FUNCTION, Era::mortal(32, 43), 43u64).encode(),
            ),
            Era::mortal(32, 43),
        );
        assert!(ux.is_signed().unwrap_or(false));
