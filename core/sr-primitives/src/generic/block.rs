...
use traits::{self, Block as BlockT, Header as HeaderT, MaybeSerialize, Member};
use Justification;
...
#[cfg(feature = "std")]
impl<'a, Header: 'a, Extrinsic: 'a + MaybeSerialize> Deserialize<'a> for Block<Header, Extrinsic>
where
    Block<Header, Extrinsic>: Decode,
{
    fn deserialize<D: Deserializer<'a>>(de: D) -> Result<Self, D::Error> {
        let r = <Vec<u8>>::deserialize(de)?;
        Decode::decode(&mut &r[..]).ok_or(::serde::de::Error::custom(
            "Invalid value passed into decode",
        ))
    }
}
...
#[cfg(feature = "std")]
impl<'a, Block: BlockT> Deserialize<'a> for SignedBlock<Block>
where
    Block::Header: 'a,
