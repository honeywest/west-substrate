    ...
    H: Debug,
{
    ...
    pub(crate) fn current_limit(&self) -> Option<N> {
        self.pending_changes
            .get(0)
            .map(|change| change.effective_number().clone())
    }
        ...
        loop {
            let remove_up_to = match self.pending_changes.first() {
                ...
                Some(change) => {
                    ...
                    if effective_number > just_finalized {
                        break;
                    }
                    ...
                    match (canonical_hash, effective_hash) {
                        (Some(canonical_hash), Some(effective_hash)) => {
                            if canonical_hash == change.canon_hash {
                                ...
                                status.new_set_block =
                                    Some((effective_hash, effective_number.clone()));
    ...
    pub fn enacts_change<F, E>(&self, just_finalized: N, mut canonical: F) -> Result<bool, E>
    where
        F: FnMut(N) -> Result<Option<H>, E>,
    {
        for change in self.pending_changes.iter() {
...
mod tests {
    ...
    fn changes_sorted_in_correct_order() {
        ...
        authorities
            .add_pending_change(change_a.clone(), ignore_existing_changes)
            .unwrap();
        authorities
            .add_pending_change(change_b.clone(), ignore_existing_changes)
            .unwrap();
        authorities
            .add_pending_change(change_c.clone(), ignore_existing_changes)
            .unwrap();

        assert_eq!(
            authorities.pending_changes,
            vec![change_a, change_c, change_b]
        );
    }
    ...
    fn disallow_multiple_changes_on_same_fork() {
        ...
        authorities
            .add_pending_change(change_a.clone(), |base| {
                is_equal_or_descendent_of(base, change_a.canon_hash)
            })
            .unwrap();

        // change b is on the same chain has the unfinalized change a so it should error
        assert!(authorities
            .add_pending_change(change_b.clone(), |base| is_equal_or_descendent_of(
                base,
                change_b.canon_hash
            ),)
            .is_err());
        ...
        authorities
            .add_pending_change(change_c.clone(), |base| {
                is_equal_or_descendent_of(base, change_c.canon_hash)
            })
            .unwrap();
        ...
        authorities
            .add_pending_change(change_b.clone(), |base| {
                is_equal_or_descendent_of(base, change_b.canon_hash)
            })
            .unwrap();
    }
}
