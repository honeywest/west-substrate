...
use super::{BlockStatus, CompactCommit, Error, SignedMessage};
...
impl<Block: BlockT, Status, I, M> Stream for UntilImported<Block, Status, I, M>
where
    Status: BlockStatus<Block>,
...
pub(crate) type UntilVoteTargetImported<Block, Status, I> =
    UntilImported<Block, Status, I, SignedMessage<Block>>;
...
pub(crate) type UntilCommitBlocksImported<Block, Status, I> =
    UntilImported<Block, Status, I, BlockCommitMessage<Block>>;
...
mod tests {
    ...
    fn blocking_commit_message() {
        ...
        commit_tx
            .unbounded_send((0, unknown_commit.clone()))
            .unwrap();
        ...
        assert_eq!(
            runtime.block_on(work).map_err(|(e, _)| e).unwrap().0,
            Some((0, unknown_commit))
        );
    }
