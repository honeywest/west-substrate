...
use client::{
    error::Result,
    runtime_api::{ApiExt, CallRuntimeAt, ConstructRuntimeApi, Core, RuntimeVersion},
    BlockchainEvents,
};
use codec::Decode;
...
use network::test::{Block, Hash, Peer, PeersClient, TestNetFactory};
...
impl Network for MessageRouting {
    ...
    fn messages_for(&self, round: u64, set_id: u64) -> Self::In {
        ...
        let messages = peer.with_spec(move |_, _| gossip.messages_for(make_topic(round, set_id)));
    ...
    fn send_message(&self, round: u64, set_id: u64, message: Vec<u8>) {
        ...
        inner
            .peer(self.peer_id)
            .gossip_message(make_topic(round, set_id), message, false);
    ...
    fn drop_messages(&self, round: u64, set_id: u64) {
        ...
        let peer = inner.peer(self.peer_id);
        let mut gossip = peer.consensus_gossip().write();
        peer.with_spec(move |_, _| gossip.collect_garbage(|t| t == &topic));
    }
    ...
    fn commit_messages(&self, set_id: u64) -> Self::In {
        ...
        let peer = inner.peer(self.peer_id);
        let mut gossip = peer.consensus_gossip().write();
        let messages = peer.with_spec(move |_, _| gossip.messages_for(make_commit_topic(set_id)));
    ...
    fn send_commit(&self, set_id: u64, message: Vec<u8>) {
        ...
        inner
            .peer(self.peer_id)
            .gossip_message(make_commit_topic(set_id), message, true);
        inner.route_until_complete();
    }
}
...
impl Core<Block> for RuntimeApi {
    ...
    fn initialise_block(&self, _: &BlockId<Block>, _: &<Block as BlockT>::Header) -> Result<()> {
        unimplemented!("Not required for testing!")
    }
}
...
impl GrandpaApi<Block> for RuntimeApi {
    fn grandpa_authorities(&self, at: &BlockId<Block>) -> Result<Vec<(AuthorityId, u64)>> {
...
fn transition_3_voters_twice_1_observer() {
    ...
    let peers_c = &[Keyring::Alice, Keyring::Eve, Keyring::Two];
