...
use client::{
    backend::Backend, error::Error as ClientError, error::ErrorKind as ClientErrorKind,
    BlockchainEvents, CallExecutor, Client,
};
use codec::{Decode, Encode};
use consensus_common::{Authorities, BlockImport, ImportBlock, ImportResult};
...
use substrate_primitives::{ed25519, AuthorityId, Blake2Hasher, H256};
use tokio::timer::Delay;

use grandpa::Error as GrandpaError;
use grandpa::{round::State as RoundState, voter, BlockNumberOps, Equivocation};

use network::consensus_gossip::ConsensusMessage;
use network::{ExHashT, Service as NetworkService};
...
#[cfg(feature = "service-integration")]
pub use service_integration::{BlockImportForService, LinkHalfForService};
...
pub type Commit<Block> =
    grandpa::Commit<<Block as BlockT>::Hash, NumberFor<Block>, ed25519::Signature, AuthorityId>;
...
impl<B: BlockT, S: network::specialization::NetworkSpecialization<B>, H: ExHashT>
    NetworkBridge<B, S, H>
{
...
impl<B: BlockT, S: network::specialization::NetworkSpecialization<B>, H: ExHashT> Clone
    for NetworkBridge<B, S, H>
{
    fn clone(&self) -> Self {
        NetworkBridge {
...
impl<B: BlockT, S: network::specialization::NetworkSpecialization<B>, H: ExHashT> Network
    for NetworkBridge<B, S, H>
{
    ...
    fn messages_for(&self, round: u64, set_id: u64) -> Self::In {
        self.service
            .consensus_gossip()
            .write()
            .messages_for(message_topic::<B>(round, set_id))
    }
    ...
    fn commit_messages(&self, set_id: u64) -> Self::In {
        self.service
            .consensus_gossip()
            .write()
            .messages_for(commit_topic::<B>(set_id))
    }
    ...
    NumberFor<Block>: BlockNumberOps,
{
    ...
    fn completed(
        ...
        let res = Backend::insert_aux(
            &**self.inner.backend(),
            &[(LAST_COMPLETED_KEY, &encoded_state[..])],
            &[],
        );
        if let Err(e) = res {
...
impl<Block: BlockT<Hash = H256>> GrandpaJustification<Block> {
        ...
        NumberFor<Block>: grandpa::BlockNumberOps,
    {
        ...
        match grandpa::validate_commit(&justification.commit, voters, None, &ancestry_chain) {
            Ok(Some(_)) => {}
            _ => {
...
) -> Result<(), ExitOrError<Block::Hash, NumberFor<Block>>>
where
    B: Backend<Block, Blake2Hasher>,
    E: CallExecutor<Block, Blake2Hasher> + Send + Sync,
    RA: Send + Sync,
...
impl<B, E, Block: BlockT<Hash = H256>, RA, PRA> BlockImport<Block>
    for GrandpaBlockImport<B, E, Block, RA, PRA>
where
    NumberFor<Block>: grandpa::BlockNumberOps,
    B: Backend<Block, Blake2Hasher> + 'static,
    ...
    PRA::Api: GrandpaApi<Block>,
{
    ...
    fn import_block(
        ...
        let just_in_case = if let Some(change) = maybe_change {
            ...
            block
                .auxiliary
                .push((AUTHORITY_SET_KEY.to_vec(), Some(authorities.encode())));
        ...
        let import_result = self
            .inner
            .import_block(block, new_authorities)
            .map_err(|e| {
...
impl<B, E, Block: BlockT<Hash = H256>, RA, PRA> Authorities<Block>
    for GrandpaBlockImport<B, E, Block, RA, PRA>
where
    B: Backend<Block, Blake2Hasher> + 'static,
    E: CallExecutor<Block, Blake2Hasher> + 'static + Clone + Send + Sync,
...
impl<B, E, Block: BlockT<Hash = H256>, RA, PRA> ProvideRuntimeApi
    for GrandpaBlockImport<B, E, Block, RA, PRA>
where
    B: Backend<Block, Blake2Hasher> + 'static,
    E: CallExecutor<Block, Blake2Hasher> + 'static + Clone + Send + Sync,
        ...
        Some(raw) => ::authorities::AuthoritySet::decode(&mut &raw[..])
            .ok_or_else(|| {
                ::client::error::ErrorKind::Backend(format!(
                    "GRANDPA authority set kept in invalid format"
                ))
            })?
            .into(),
    };
...
fn committer_communication<Block: BlockT<Hash = H256>, B, E, N, RA>(
    ...
    impl Stream<
        Item = (
            u64,
            ::grandpa::CompactCommit<H256, NumberFor<Block>, ed25519::Signature, AuthorityId>,
        ),
    ...
    impl Sink<
        SinkItem = (
            u64,
            ::grandpa::Commit<H256, NumberFor<Block>, ed25519::Signature, AuthorityId>,
        ),
    ...
    NumberFor<Block>: BlockNumberOps,
{
    ...
    let commit_out = ::communication::CommitsOut::<Block, _>::new(network.clone(), set_id);
    ...
    RA: Send + Sync + 'static,
{
    ...
    let (last_round_number, last_state) =
        match Backend::get_aux(&**client.backend(), LAST_COMPLETED_KEY)? {
            None => (
                0,
                RoundState::genesis((genesis_hash, <NumberFor<Block>>::zero())),
            ),
            Some(raw) => LastCompleted::decode(&mut &raw[..]).ok_or_else(|| {
                ::client::error::ErrorKind::Backend(format!(
                    "Last GRANDPA round state kept in invalid format"
                ))
            })?,
        };
    ...
    let initial_state = (
        ...
        last_round_number,
        last_state,
        authority_set_change.into_future(),
    );
    let voter_work = future::loop_fn(initial_state, move |params| {
        ...
        let committer_data = committer_communication(env.set_id, &env.voters, &client, &network);
        ...
        future::Either::A(
            voter
                ...
                .then(move |res| match res {
                    ...
                    Err(future::Either::A((
                        ExitOrError::AuthoritiesChanged(new),
                        authority_set_change,
                    ))) => {
